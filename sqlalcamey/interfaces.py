...
def get_all_artifacts_associated_with_benchmark(session: Session, benchmark_id: int):
    """
    Retrieves all artifacts associated with a specific benchmark.

    This function queries the BenchmarkArtifacts table for all rows with the given benchmark_id.
    Then, for each of these BenchmarkArtifacts, it queries the Artifact table for the Artifact
    with the artifact_id from the BenchmarkArtifacts. The result is a list of all Artifact objects
    associated with the given benchmark.

    Args:
        session (Session): The session to use for making the database queries.
        benchmark_id (int): The ID of the benchmark to retrieve artifacts for.

    Returns:
        list[Artifact]: A list of Artifact objects associated with the given benchmark.
    ...
    Example:
        ...
        >>> session = Session()
        >>> benchmark_id = 1
        >>> artifacts = get_all_artifacts_associated_with_benchmark(session, benchmark_id)
        >>> print(artifacts)
        [<Artifact(name='Artifact1', location='Location1', ...)>, <Artifact(name='Artifact2', location='Location2', ...)>]
    """
    # Query for all BenchmarkArtifacts with the given benchmark_id
    benchmark_artifacts = (
        session.query(BenchmarkArtifacts)
        .filter(BenchmarkArtifacts.benchmark_id == benchmark_id)
        .all()
    ...
    # Now, for each BenchmarkArtifacts, get the associated Artifact
    artifacts = [
        session.query(Artifact).get(ba.artifact_id) for ba in benchmark_artifacts
    ]
