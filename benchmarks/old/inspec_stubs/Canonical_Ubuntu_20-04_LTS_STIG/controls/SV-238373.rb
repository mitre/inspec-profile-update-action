...
If "pam_lastlog" is missing from "/etc/pam.d/login" file, is not "required", or the "silent" option is present, this is a finding.'
  desc 'fix', 'Configure the Ubuntu operating system to provide users with feedback on when account accesses last occurred by setting the required configuration options in "/etc/pam.d/login".  
...
session     required      pam_lastlog.so showfailed'
  ...
  tag check_id: 'C-41583r654292_chk'
  tag severity: 'low'
  tag gid: 'V-238373'
