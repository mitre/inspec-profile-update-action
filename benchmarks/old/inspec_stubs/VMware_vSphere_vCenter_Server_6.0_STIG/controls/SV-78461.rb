control 'SV-78461' do
  ...
  desc 'check', 'From the vSphere Web Client go to Networking >> Select a distributed port group >> Manage >> Settings >> Properties.  View the Properties pane and verify all Override port policies are set to disabled.
...
If any port level overrides are enabled and not documented, this is a finding.'
  desc 'fix', 'From the vSphere Web Client go to Networking >> Select a distributed port group >> Manage >> Settings >> Properties.  Click Edit and change all Override port policies to disabled.
...
$pg.ReconfigureDVPortgroup_Task($spec)
  ...
  tag check_id: 'C-64723r1_chk'
  tag severity: 'low'
  tag gid: 'V-63971'
  tag rid: 'SV-78461r1_rule'
  tag stig_id: 'VCWN-06-000017'
  tag gtitle: 'SRG-APP-000516'
  tag fix_id: 'F-69901r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
