control 'SV-45314' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone "Allow updates to status bar via script" must be "Enabled", and "Disable" selected from the drop-down box. 
...
Criteria: If the value 2103 is REG_DWORD = 3, this is not a finding.'
  ...
  tag check_id: 'C-42662r1_chk'
  tag severity: 'medium'
  tag gid: 'V-22634'
  tag rid: 'SV-45314r1_rule'
  tag stig_id: 'DTBI910'
  tag gtitle: 'DTBI910 - Status bar updates via script - Internet'
  tag fix_id: 'F-38710r1_fix'
  tag 'documentable'
