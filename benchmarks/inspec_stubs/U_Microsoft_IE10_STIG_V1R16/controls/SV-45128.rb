...
AND
Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Url History 

Criteria: If the value DaysToKeep is REG_DWORD = 40 (decimal), this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Delete Browsing History -> "Disable Configuring History" to "Enabled", and enter "40" in 'Days to keep pages in History'.)
  impact 0.5
  ref 'DPMS Target IE Version 10'
  tag check_id: 'C-42475r1_chk'
  tag severity: 'medium'
  tag gid: 'V-21887'
  tag rid: 'SV-45128r1_rule'
  tag stig_id: 'DTBI300'
  tag gtitle: 'DTBI300 - Configuring History lists'
  tag fix_id: 'F-38525r1_fix'
  tag 'documentable'
