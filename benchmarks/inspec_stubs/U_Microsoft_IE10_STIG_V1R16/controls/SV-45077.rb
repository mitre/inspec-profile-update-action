control 'SV-45077' do
  ...
  desc 'This policy setting allows you to turn off the ActiveX opt-in prompt. The ActiveX opt-in prevents Websites from loading any COM object without prior approval. If a page attempts to load a COM object that Internet Explorer has not used before, an information bar will appear asking the user for approval. If you enable this policy setting, the ActiveX opt-in prompt will not appear. Internet Explorer does not ask the user for permission to load a control, and will load the ActiveX if it passes all other internal security checks. If you disable or do not configure this policy setting the ActiveX opt-in prompt will appear.'
...
Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Ext 
...
Criteria: If the value NoFirsttimeprompt is REG_DWORD = 1, this is not a finding.'
  ...
  tag check_id: 'C-42449r1_chk'
  tag severity: 'medium'
  tag gid: 'V-30778'
  tag rid: 'SV-45077r1_rule'
  tag stig_id: 'DTBI805'
  tag gtitle: 'DTBI805 - Opt-In Prompts for ActiveX'
  tag fix_id: 'F-38484r1_fix'
  tag 'documentable'
