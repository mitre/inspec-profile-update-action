control 'SV-45075' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> " Allow only approved domains to use ActiveX controls without prompt" must be "Enabled", and "Enable" selected from the drop-down box. 
...
Criteria: If the value 120b is REG_DWORD = 3, this is not a finding.'
  ...
  tag check_id: 'C-42447r1_chk'
  tag severity: 'medium'
  tag gid: 'V-22155'
  tag rid: 'SV-45075r1_rule'
  tag stig_id: 'DTBI830'
  tag gtitle: 'DTBI830 - ActiveX controls no prompt - Internet'
  tag fix_id: 'F-38482r1_fix'
  tag 'documentable'
