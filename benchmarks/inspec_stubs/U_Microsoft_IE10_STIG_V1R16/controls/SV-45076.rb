control 'SV-45076' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> "Allow only approved domains to use ActiveX controls without prompt" must be "Enabled", and "Enable" selected from the drop-down box. 
...
Criteria: If the value 120b is REG_DWORD = 3, this is not a finding.'
  ...
  tag check_id: 'C-42448r1_chk'
  tag severity: 'medium'
  tag gid: 'V-22160'
  tag rid: 'SV-45076r1_rule'
  tag stig_id: 'DTBI880'
  tag gtitle: 'DTBI880 - ActiveX controls no prompt - Restricted'
  tag fix_id: 'F-38483r1_fix'
  tag 'documentable'
