control 'SV-45157' do
  title 'First-Run prompt ability must be disallowed (Restricted Sites zone).'
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> "Turn off first-run prompt" must be "Enabled", and "Disable" selected from the drop-down box. 
...
Criteria: If the value 1208 is REG_DWORD = 3, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> "Turn off first-run prompt" to "Enabled", and select "Disable" from the drop-down box.'
  impact 0.5
  ref 'DPMS Target IE Version 10'
  tag check_id: 'C-42500r2_chk'
  tag severity: 'medium'
  tag gid: 'V-15526'
  tag rid: 'SV-45157r1_rule'
  tag stig_id: 'DTBI480'
  tag gtitle: 'DTBI480 - First-Run Opt-In - Restricted'
  tag fix_id: 'F-38553r2_fix'
  tag 'documentable'
