control 'SV-53238' do
  ...
  desc 'check', %q(Verify the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Navigate URL" must be "Enabled" and a check in the 'mspub.exe' check box must be present.
...
Criteria: If the value mspub.exe is REG_DWORD = 1, this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Navigate URL" to "Enabled" and place a check in the 'mspub.exe' check box.)
  impact 0.5
  ref 'DPMS Target Microsoft Publisher 2013'
  tag check_id: 'C-47545r1_chk'
  tag severity: 'medium'
  tag gid: 'V-40895'
  tag rid: 'SV-53238r1_rule'
  tag stig_id: 'DTOO123'
  tag gtitle: 'DTOO123 - Block Navigation to URL from Office'
  tag fix_id: 'F-46165r1_fix'
  tag 'documentable'
