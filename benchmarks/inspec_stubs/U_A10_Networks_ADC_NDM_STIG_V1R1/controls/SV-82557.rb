...
The A10 Networks ADC records in the audit log when an account is removed. This appears as the command that created the account and contains the keyword "admin". These messages must be forwarded to the ISSO and administrators. Configuring the device to forward all audit log messages to an actively monitored syslog server or SNMP management station meets this requirement.'
  desc 'check', 'The A10 Networks ADC records in the audit log when an account is removed. This appears as the command that created the account and contains the keyword "admin". These messages must be forwarded to the ISSO and administrators. These messages must be forwarded to the ISSO and administrators. This is met by sending audit log messages to the Syslog servers or SNMP management station, which is continuously monitored.
...
"facility-name" is the name of a log facility.'
  ...
  tag check_id: 'C-68627r1_chk'
  tag severity: 'medium'
  tag gid: 'V-68067'
  tag rid: 'SV-82557r1_rule'
  tag stig_id: 'AADC-NM-000081'
  tag gtitle: 'SRG-APP-000294-NDM-000278'
  tag fix_id: 'F-74183r1_fix'
  tag 'documentable'
  tag cci: ['CCI-001686']
  tag nist: ['AC-2 (4)']
end
