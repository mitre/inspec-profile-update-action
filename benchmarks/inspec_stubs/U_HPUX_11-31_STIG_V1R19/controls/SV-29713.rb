control 'SV-29713' do
  ...
  desc 'check', 'Check the system for an IP Filter (IPF) rule blocking incoming source-routed packets.
...
If the listed rules do not block incoming traffic with both lsrr and ssrr options, this is a finding.'
  desc 'fix', 'Edit /etc/opt/ipf/ipf.conf and add rules to block incoming source-routed packets, such as:
...
# ipf -Fa -A -f /etc/opt/ipf/ipf.conf'
  ...
  tag check_id: 'C-36509r2_chk'
  tag severity: 'medium'
  tag gid: 'V-22414'
  tag rid: 'SV-29713r2_rule'
  tag stig_id: 'GEN003607'
  tag gtitle: 'GEN003607'
  tag fix_id: 'F-31869r2_fix'
  tag 'documentable'
