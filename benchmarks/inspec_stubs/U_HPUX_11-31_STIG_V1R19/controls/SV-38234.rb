control 'SV-38234' do
  ...
  desc 'check', 'HP-UX 11-v3 may be booted from the following system startup media (must have been previously configured by root):
•	Hard drives
...
Determine if the system is configured to boot from devices other than the system startup media. Verification should (optimally) be performed during IPL/ISL boot. In lieu of rebooting the system, ask the SA if the system is configured to boot from devices other than system startup media.  If so, this is a finding.'
  desc 'fix', 'Configure the system to only boot from system startup media. The system must first be rebooted in order to set primary and alternate boot paths for the desired system startup media.'
  impact 0.7
  ref 'DPMS Target HP-UX 11.31'
  tag check_id: 'C-36795r1_chk'
  tag severity: 'high'
  tag gid: 'V-1013'
  tag rid: 'SV-38234r1_rule'
  tag stig_id: 'GEN008600'
  tag gtitle: 'GEN008600'
  tag fix_id: 'F-32173r1_fix'
  tag 'documentable'
