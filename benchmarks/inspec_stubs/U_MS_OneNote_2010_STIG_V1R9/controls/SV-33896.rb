...
This functionality can be controlled separately for instances of Internet Explorer spawned by Office applications (for example, if a user clicks a link in an Office document or selects a menu option that loads a Web page). If user names and passwords in URLs are allowed, users could be diverted to dangerous Web pages, which could pose a security risk.'
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Disable user name and password” must be “Enabled” and a check in the ‘onent.exe’ check box must be present.
...
Criteria: If the value onenote.exe is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Disable user name and password” to “Enabled” and place a check in the ‘onent.exe’ check box.'
  impact 0.5
  ref 'DPMS Target Microsoft OneNote 2010'
  tag check_id: 'C-34308r4_chk'
  tag severity: 'medium'
  tag gid: 'V-17173'
  tag rid: 'SV-33896r2_rule'
  tag stig_id: 'DTOO104 - OneNote'
  tag gtitle: 'DTOO104 - Disable user name and password'
  tag fix_id: 'F-29967r1_fix'
  tag 'documentable'
