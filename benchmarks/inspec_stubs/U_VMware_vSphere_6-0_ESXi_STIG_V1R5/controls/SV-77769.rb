...
Even though VMware believes information being disclosed in real world conditions is unrealistic, out of an abundance of caution upcoming ESXi Update releases will no longer enable TPS between Virtual Machines by default (TPS will still be utilized within individual VMs).'
  desc 'check', 'From the vSphere Client select the ESXi Host and go to Configuration >> Advanced Settings.  Select the Mem.ShareForceSalting value and verify it is set to 2.
...
If the Mem.ShareForceSalting setting is not set to 2, this is a finding.'
  desc 'fix', 'From the vSphere Client select the ESXi Host and go to Configuration >> Advanced Settings.  Select the Mem.ShareForceSalting value and configure it to 2.
...
Get-VMHost | Get-AdvancedSetting -Name Mem.ShareForceSalting | Set-AdvancedSetting -Value 2'
  ...
  tag check_id: 'C-64013r1_chk'
  tag severity: 'low'
  tag gid: 'V-63279'
  tag rid: 'SV-77769r1_rule'
  tag stig_id: 'ESXI-06-000055'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-69197r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
