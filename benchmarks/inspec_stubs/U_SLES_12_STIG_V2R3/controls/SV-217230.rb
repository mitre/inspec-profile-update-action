control 'SV-217230' do
  title 'The SUSE operating system must generate audit records for all uses of the open command.'
  desc 'Without generating audit records that are specific to the security and mission needs of the organization, it would be difficult to establish, correlate, and investigate the events relating to an incident or identify those responsible for one.

Audit records can be generated from various components within the information system (e.g., module or policy filter).

'
  desc 'check', 'Verify the SUSE operating system generates an audit record for all uses of the "open" command.
...
If the output does not produce rules containing "-F exit=-EACCES", this is a finding.'
  desc 'fix', 'Configure the SUSE operating system to generate an audit record for all uses of the "open" command. 
...
# sudo systemctl restart auditd.service'
  ...
  tag check_id: 'C-18458r369846_chk'
  tag severity: 'medium'
  tag gid: 'V-217230'
  tag rid: 'SV-217230r603262_rule'
  tag stig_id: 'SLES-12-020490'
  tag gtitle: 'SRG-OS-000037-GPOS-00015'
  tag fix_id: 'F-18456r369847_fix'
