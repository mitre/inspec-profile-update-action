control 'SV-258415' do
  title 'Google Android 14 must be configured to enforce an application installation policy by specifying an application allowlist that restricts applications by the following characteristics: [selection: list of digital signatures, cryptographic hash values, names, application version].'
...
SFR ID: FMT_SMF_EXT.1.1 #8b'
  desc 'check', 'Review managed Google Android 14 device configuration settings to determine if the mobile device has an application allowlist configured. Verify all applications listed on the allowlist have been approved by the Approving Official (AO).
...
Note: The application allowlist will include approved core applications (included in the OS by the OS vendor) and pre-installed applications (provided by the MD vendor and wireless carrier), or the MD must provide an alternate method of restricting user access/execution to core and pre-installed applications. For Google Android, there are no pre-installed applications.'
  desc 'fix', 'Configure the Google Android 14 device to use an application allowlist.
...
Note: Managed Google Play is an allowed App Store.'
  ...
  tag check_id: 'C-62156r928268_chk'
  tag severity: 'medium'
  tag gid: 'V-258415'
  tag rid: 'SV-258415r928270_rule'
  tag stig_id: 'GOOG-14-006600'
  tag gtitle: 'PP-MDF-333060'
  tag fix_id: 'F-62080r928269_fix'
  tag 'documentable'
  tag cci: ['CCI-001764']
  tag nist: ['CM-7 (2)']
end
