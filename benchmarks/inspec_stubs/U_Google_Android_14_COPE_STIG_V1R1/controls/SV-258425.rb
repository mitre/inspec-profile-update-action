control 'SV-258425' do
  title 'Google Android 14 must be configured to disable exceptions to the access control policy that prevent [selection: application processes, groups of application processes] from accessing [selection: all, private] data stored by other [selection: application processes, groups of application processes].'
...
If the EMM console device policy is not set to disable data sharing between profiles, this is a finding.'
  desc 'fix', 'Configure the Google Android 14 device to enable the access control policy that prevents [selection: application processes, groups of application processes] from accessing [selection: all, private] data stored by other [selection: application processes, groups of application processes].
...
3. Toggle "Disallow cross profile copy/paste" to "ON".'
  ...
  tag check_id: 'C-62166r928298_chk'
  tag severity: 'medium'
  tag gid: 'V-258425'
  tag rid: 'SV-258425r928300_rule'
  tag stig_id: 'GOOG-14-008900'
  tag gtitle: 'PP-MDF-333280'
  tag fix_id: 'F-62090r928299_fix'
  tag 'documentable'
