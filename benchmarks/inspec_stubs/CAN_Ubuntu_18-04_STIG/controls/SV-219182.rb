control 'SV-219182' do
  ...
  desc 'The Ubuntu operating system must use a FIPS-compliant hashing algorithm to securely store the password. The FIPS-compliant hashing algorithm parameters must be selected in order to harden the system against offline attacks.'
  desc 'check', 'Verify that encrypted passwords stored in /etc/shadow use a strong cryptographic hash.
...
If the output does not contain "sha512", or it is commented out, this is a finding.'
  desc 'fix', 'Configure the Ubuntu operating system to encrypt all stored passwords with a strong cryptographic hash.
...
Edit/modify /etc/login.defs and set "ENCRYPT_METHOD sha512".'
  ...
  tag check_id: 'C-20907r304874_chk'
  tag severity: 'medium'
  tag gid: 'V-219182'
  tag rid: 'SV-219182r610963_rule'
  tag stig_id: 'UBTU-18-010110'
  tag gtitle: 'SRG-OS-000120-GPOS-00061'
  tag fix_id: 'F-20906r304875_fix'
  tag 'documentable'
  tag legacy: ['SV-109695', 'V-100591']
  tag cci: ['CCI-000803']
  tag nist: ['IA-7']
end
