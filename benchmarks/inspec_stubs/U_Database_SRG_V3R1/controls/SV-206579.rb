...
This calls for inspection of application source code, which will require collaboration with the application developers. It is recognized that in many cases, the database administrator (DBA) is organizationally separate from the application developers, and may have limited, if any, access to source code. Nevertheless, protections of this type are so important to the secure operation of databases that they must not be ignored. At a minimum, the DBA must attempt to obtain assurances from the development organization that this issue has been addressed, and must document what has been discovered.)
  desc 'check', 'Check DBMS settings and custom database code to determine if detailed error messages are ever displayed to unauthorized individuals.

If detailed error messages are displayed to individuals not authorized to view them, this is a finding.'
  desc 'fix', 'Configure DBMS settings, custom database code, and associated application code not to display detailed error messages to those not authorized to view them.'
  impact 0.5
  ref 'DPMS Target Database Generic'
  tag check_id: 'C-6839r291405_chk'
  tag severity: 'medium'
  tag gid: 'V-206579'
  tag rid: 'SV-206579r617447_rule'
  tag stig_id: 'SRG-APP-000267-DB-000163'
  tag gtitle: 'SRG-APP-000267'
  tag fix_id: 'F-6839r291406_fix'
  tag 'documentable'
  tag legacy: ['SV-42908', 'V-32571']
  tag cci: ['CCI-001314']
  tag nist: ['SI-11 b']
end
