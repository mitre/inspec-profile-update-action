...
ALGs may provide session control functionality as part of content filtering, load balancing, or proxy services.'
  desc 'check', 'Verify the BIG-IP Core is configured to terminate all communications at the end of the session as follows:
...
- For user sessions (nonprivileged sessions), the session must be terminated after 15 minutes of inactivity.'
  desc 'fix', 'Configure BIG-IP Core to terminate all communications session at the end of the session or as follows: 

- For in-band management sessions (privileged sessions), the session must be terminated after 10 minutes of inactivity.
- For user sessions (nonprivileged sessions), the session must be terminated after 15 minutes of inactivity.'
  impact 0.5
  ref 'DPMS Target F5 BIG-IP Local Traffic Manager 11.x'
  tag check_id: 'C-16957r939127_chk'
  tag severity: 'medium'
  tag gid: 'V-215765'
  tag rid: 'SV-215765r939149_rule'
  tag stig_id: 'F5BI-LT-000093'
  tag gtitle: 'SRG-NET-000213-ALG-000107'
  tag fix_id: 'F-16955r939128_fix'
  tag 'documentable'
  tag legacy: ['V-60311', 'SV-74741']
