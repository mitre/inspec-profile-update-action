control 'SV-219163' do
  ...
  desc 'check', 'If smart card authentication is not being used on the system this item is Not Applicable.
...
If "offline_credentials_expiration" is not set to a value of "1", in /etc/sssd/sssd.conf or in a file with a name ending in .conf in the /etc/sssd/conf.d/ directory, this is a finding.'
  desc 'fix', 'Configure Pluggable Authentication Module (PAM) to prohibit the use of cached authentications after one day. Add or change the following line in "/etc/sssd/sssd.conf" just below the line "[pam]".
