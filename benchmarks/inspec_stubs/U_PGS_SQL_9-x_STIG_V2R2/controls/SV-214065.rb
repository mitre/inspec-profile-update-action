...
-- Bear in mind that all this applies not only to screen input, but also to the values in an incoming message to a web service or to a stored procedure called by a software component that has not itself been hardened in these ways. Not only can the caller be subject to such vulnerabilities; it may itself be the attacker."
  desc 'check', 'Review PostgreSQL source code (trigger procedures, functions) and application source code to identify cases of dynamic code execution.

If dynamic code execution is employed without protective measures against code injection, this is a finding.'
  desc 'fix', 'Where dynamic code execution is used, modify the code to implement protections against code injection (IE: prepared statements).'
  impact 0.5
  ref 'DPMS Target PostgreSQL 9.x'
  tag check_id: 'C-15281r360826_chk'
  tag severity: 'medium'
  tag gid: 'V-214065'
  tag rid: 'SV-214065r508027_rule'
  tag stig_id: 'PGS9-00-002000'
  tag gtitle: 'SRG-APP-000251-DB-000392'
  tag fix_id: 'F-15279r360827_fix'
  tag 'documentable'
  tag legacy: ['SV-87527', 'V-72875']
  tag cci: ['CCI-001310']
  tag nist: ['SI-10']
end
