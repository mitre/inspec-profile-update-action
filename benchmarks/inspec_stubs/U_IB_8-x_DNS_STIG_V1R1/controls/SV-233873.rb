...
To protect internal DNS resource information, it is important to isolate the requests to internal DNS servers. Separating internal and external roles in DNS prevents address space that is private (e.g., 10.0.0.0/24) or is otherwise concealed by some form of Network Address Translation from leaking into the public DNS system.'
  desc 'check', 'Review the Infoblox Grid configuration and network architecture to verify that the appropriate zones are served by the correct internal or external member(s).  
...
If an external server contains internal data, or vice versa, this is a finding.'
  desc 'fix', '1. Navigate to Data Management >> DNS >> Zones and Members.  
...
4. Perform a service restart if necessary.'
  ...
  tag check_id: 'C-37058r611139_chk'
  tag severity: 'medium'
  tag gid: 'V-233873'
  tag rid: 'SV-233873r621666_rule'
  tag stig_id: 'IDNS-8X-400015'
  tag gtitle: 'SRG-APP-000516-DNS-000101'
  tag fix_id: 'F-37023r611140_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
