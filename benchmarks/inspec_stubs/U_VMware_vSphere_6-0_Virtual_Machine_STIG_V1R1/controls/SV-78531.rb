control 'SV-78531' do
  ...
  desc 'check', 'From the vSphere Client select the Virtual Machine right click and go to Edit Settings >> VM Options Tab >> Advanced >> Configuration Parameters.  Find the isolation.tools.dnd.disable value and verify it is set to true.
...
If the virtual machine advanced setting isolation.tools.dnd.disable does not exist or is not set to true, this is a finding.'
  desc 'fix', 'From the vSphere Client select the Virtual Machine right click and go to Edit Settings >> VM Options Tab >> Advanced >> Configuration Parameters.  Find the isolation.tools.dnd.disable value and set it to true.  If the setting does not exist click "Add Row" to add the setting to the virtual machine.
...
Get-VM "VM Name" | Get-AdvancedSetting -Name isolation.tools.dnd.disable | Set-AdvancedSetting -Value true'
  ...
  tag check_id: 'C-64791r1_chk'
  tag severity: 'low'
  tag gid: 'V-64041'
  tag rid: 'SV-78531r1_rule'
  tag stig_id: 'VMCH-06-000002'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-69969r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
