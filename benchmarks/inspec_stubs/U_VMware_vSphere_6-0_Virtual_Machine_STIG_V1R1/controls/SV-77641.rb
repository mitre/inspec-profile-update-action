control 'SV-77641' do
  ...
  desc 'check', 'From the vSphere Client select the Virtual Machine right click and go to Edit Settings >> VM Options Tab >> Advanced >>  Configuration Parameters.  Find the isolation.tools.copy.disable value and verify it is set to true.
...
If the virtual machine advanced setting isolation.tools.copy.disable does not exist or is not set to true, this is a finding.'
  desc 'fix', 'From the vSphere Client select the Virtual Machine right click and go to Edit Settings >> VM Options Tab >> Advanced >> Configuration Parameters.
...
Get-VM "VM Name" | Get-AdvancedSetting -Name isolation.tools.copy.disable | Set-AdvancedSetting -Value true'
  ...
  tag check_id: 'C-63903r1_chk'
  tag severity: 'low'
  tag gid: 'V-63151'
  tag rid: 'SV-77641r1_rule'
  tag stig_id: 'VMCH-06-000001'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-69069r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
