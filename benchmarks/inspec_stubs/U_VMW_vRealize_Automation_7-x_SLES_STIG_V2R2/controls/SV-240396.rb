...
Password complexity is one factor of several that determines how long it takes to crack a password. The more complex the password, the greater the number of possible combinations that need to be tested before the password is compromised.'
  desc 'check', 'Check that the SLES for vRealize enforces password complexity by requiring that at least one numeric character be used by running the following command:
...
password requisite pam_cracklib.so dcredit=-1 ucredit=-1 lcredit=-1 ocredit=-1 minlen=14 difok=4 retry=3'
  desc 'fix', %q(If "dcredit" was not set at all in /etc/pam.d/common-password-vmware.local, run the following command:
...
# sed -i '/pam_cracklib.so/ s/dcredit=../dcredit=-1/' /etc/pam.d/common-password-vmware.local)
  ...
  tag check_id: 'C-43629r670927_chk'
  tag severity: 'medium'
  tag gid: 'V-240396'
  tag rid: 'SV-240396r670929_rule'
  tag stig_id: 'VRAU-SL-000355'
  tag gtitle: 'SRG-OS-000071-GPOS-00039'
  tag fix_id: 'F-43588r670928_fix'
  tag 'documentable'
  tag legacy: ['SV-100219', 'V-89569']
  tag cci: ['CCI-000194']
  tag nist: ['IA-5 (1) (a)']
end
