control 'SV-230439' do
  title 'Successful/unsuccessful uses of the rename command in RHEL 8 must generate an audit record.'
...
'
  desc 'check', 'Verify RHEL 8 generates an audit record when successful/unsuccessful attempts to use the "rename" command by performing the following command to check the file system rules in "/etc/audit/audit.rules":
...
If the command does not return a line, or the line is commented out, this is a finding.'
  desc 'fix', 'Configure the audit system to generate an audit event for any successful/unsuccessful use of the "rename" command by adding or updating the following rules in the "/etc/audit/rules.d/audit.rules" file:
...
The audit daemon must be restarted for the changes to take effect.'
  ...
  tag check_id: 'C-33108r568063_chk'
  tag severity: 'medium'
  tag gid: 'V-230439'
