control 'SV-93359' do
  ...
  desc 'The Tanium Server can use either a SQL Server RDBMS installed locally to the same device as the Tanium Server application or a remote dedicated or shared SQL Server instance. Using a local SQL Server database typically requires no changes to network firewall rules since all communication remains on the Tanium Server device. To access database resources installed to a remote device, however, the Tanium Server service communicates over the port reserved for SQL, by default port 1433, to the database.
...
Configure the network firewall to allow the above traffic.'
  ...
  tag check_id: 'C-78223r1_chk'
  tag severity: 'medium'
  tag gid: 'V-78653'
  tag rid: 'SV-93359r1_rule'
  tag stig_id: 'TANS-DB-000005'
  tag gtitle: 'SRG-APP-000383'
  tag fix_id: 'F-85389r1_fix'
  tag 'documentable'
  tag cci: ['CCI-001762']
  tag nist: ['CM-7 (1) (b)']
end
