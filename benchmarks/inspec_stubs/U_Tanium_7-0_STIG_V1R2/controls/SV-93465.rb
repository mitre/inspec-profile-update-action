control 'SV-93465' do
  title 'Tanium service must be protected from being stopped by a non-privileged user.'
...
A web server not properly tuned may become overwhelmed and cause a DoS condition even with expected traffic from users. To avoid a DoS, the web server must be tuned to handle the expected traffic for the hosted applications.'
  desc 'check', %q(Verify that to prevent a non-privileged user from impacting the Tanium Server's ability to operate, the control of the service is restricted to the Local Administrators. 
...
Run the above on all other Tanium Servers, to include Tanium Servers in an Active-Active pair.'
  ...
  tag check_id: 'C-78335r1_chk'
  tag severity: 'medium'
  tag gid: 'V-78759'
  tag rid: 'SV-93465r1_rule'
  tag stig_id: 'TANS-SV-000068'
  tag gtitle: 'SRG-APP-000435'
  tag fix_id: 'F-85501r1_fix'
  tag 'documentable'
  tag cci: ['CCI-002385']
  tag nist: ['SC-5 a']
end
