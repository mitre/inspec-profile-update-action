control 'SV-33865' do
  ...
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft Word 2010 -> Word Options -> Security -> Trust Center -> Protected View “Do not open files in unsafe locations in Protected View” must be set to "Disabled".
...
Criteria: If the value DisableUnsafeLocationsInPV is REG_DWORD = 0, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Word 2010 -> Word Options -> Security -> Trust Center -> Protected View “Do not open files in unsafe locations in Protected View” to "Disabled".'
  impact 0.5
  ref 'DPMS Target Microsoft Word 2010'
  tag check_id: 'C-34263r1_chk'
  tag severity: 'medium'
  tag gid: 'V-26615'
  tag rid: 'SV-33865r1_rule'
  tag stig_id: 'DTOO288 - Word'
  tag gtitle: 'DTOO288 - Files in unsafe locations'
  tag fix_id: 'F-29954r1_fix'
  tag 'documentable'
