control 'SV-33863' do
  ...
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft Word 2010 -> Word Options -> Security -> Trust Center -> Protected View “Do not open files from the Internet zone in Protected View” must be set to "Disabled". 
...
Criteria: If the value DisableInternetFilesInPV is REG_DWORD = 0, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Word 2010 -> Word Options -> Security -> Trust Center -> Protected View “Do not open files from the Internet zone in Protected View” to "Disabled".'
  impact 0.5
  ref 'DPMS Target Microsoft Word 2010'
  tag check_id: 'C-34260r1_chk'
  tag severity: 'medium'
  tag gid: 'V-26614'
  tag rid: 'SV-33863r1_rule'
  tag stig_id: 'DTOO121 - Word'
  tag gtitle: 'DTOO121 - Files from the Internet zone'
  tag fix_id: 'F-29953r1_fix'
  tag 'documentable'
