...
This functionality can be controlled separately for instances of Internet Explorer spawned by Office applications (for example, if a user clicks a link in an Office document or selects a menu option that loads a Web page). A security risk could occur if potentially dangerous controls are allowed to load.'
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Bind to Object” must be “Enabled” and a check in the ‘winword.exe’ check box must be present.
...
Criteria: If the value winword.exe is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Bind to Object” to “Enabled” and place a check in the ‘winword.exe’ check box.'
  impact 0.5
  ref 'DPMS Target Microsoft Word 2010'
  tag check_id: 'C-33875r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17174'
  tag rid: 'SV-33391r1_rule'
  tag stig_id: 'DTOO111 - Word'
  tag gtitle: 'DTOO111 - Enable IE Bind to Object'
  tag fix_id: 'F-29564r1_fix'
  tag 'documentable'
