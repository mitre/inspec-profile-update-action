control 'SV-257113' do
  title 'The Apple iOS/iPadOS 16 allow list must be configured to not include applications with the following characteristics: 
...
In the Apple iOS/iPadOS management tool, verify "Allow Listed App" (allowlistedAppBundleIDs) is configured and there are no managed apps with prohibited characteristics.

If "Allow listed apps" is not configured and contains apps with prohibited characteristics, this is a finding.'
  desc 'fix', 'Install a configuration profile with an allow list of approved apps (allowlistedAppBundleIDs). Ensure the allow list does not include apps with the following characteristics:
...
- Allows unencrypted (or encrypted but not FIPS 140-2/FIPS 140-3 validated) data sharing with other MDs or printers.'
  ...
  tag check_id: 'C-60798r904237_chk'
  tag severity: 'medium'
  tag gid: 'V-257113'
  tag rid: 'SV-257113r904239_rule'
  tag stig_id: 'AIOS-16-707400'
  tag gtitle: 'PP-MDF-333070'
  tag fix_id: 'F-60739r904238_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
