control 'SV-80237' do
  title 'The BlackBerry MDM Agent must be configured to generate an audit record of required warning level events, which may include: (See Vulnerability Discussion for list). This requirement only applies to Work space only and Work and personal - Regulated activation types and to version 10.3.3 and later of the BlackBerry OS.'
...
SFR ID: FAU_GEN.1.1(2) Refinement, MDM Agent EP'
  desc 'check', 'Review BlackBerry OS 10.3 configuration settings to determine if the BlackBerry is configured to generate an audit record of required warning level events, which may include Start-up and shutdown of the audit functions, Change in MDM policy, Device modification commanded by the MDM server, and Specifically defined auditable events in Table 7 of MDM Agent EP v.2.0. This procedure is performed on only on the BES console.
...
6. Verify "Warning event logging" is selected.

If the BES IT policy rule "Warning event logging" is not selected, this is a finding.
...
8. Select the check box next to the IT Policy "Warning event logging".
9. Click "Save".

Note: Procedures above are for BES 12 only, and is not available on BES 10.'
  impact 0.3
  ref 'DPMS Target BlackBerry OS 10.3.x'
  tag check_id: 'C-66423r2_chk'
  tag severity: 'low'
  tag gid: 'V-65747'
  tag rid: 'SV-80237r2_rule'
  tag stig_id: 'BB10-3X-020260'
  tag gtitle: 'PP-MDM-203001'
  tag fix_id: 'F-71811r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
