control 'SV-80207' do
  ...
  desc 'NFC data transfers do not use FIPS validated encryption. Therefore data transfer via NFC must be disabled to mitigate the possible loss of sensitive DoD information.

SFR ID: FMT_SMF_EXT.1.1 #45'
  desc 'check', 'Review BlackBerry OS 10.3 configuration settings to determine if the BlackBerry implements the management setting: disable the transfer of any file-based data via Near Field Communication (NFC) via centrally managed policy. This procedure is performed on only on the BES console.
...
6. Verify "Allow transfer of work data using NFC" is not selected.

If the BES IT policy rule "Allow transfer of work data using NFC" is selected, this is a finding.
...
7. Unselect the check box next to the IT Policy "Transfer Work Data Using NFC".
8. Click "Save".

Note: Procedures above are for BES 12 only. BES 10 procedures may be slightly different.'
  impact 0.5
  ref 'DPMS Target BlackBerry OS 10.3.x'
  tag check_id: 'C-66373r2_chk'
  tag severity: 'medium'
  tag gid: 'V-65717'
  tag rid: 'SV-80207r1_rule'
  tag stig_id: 'BB10-3X-000990'
  tag gtitle: 'PP-MDF-991000'
  tag fix_id: 'F-71761r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
