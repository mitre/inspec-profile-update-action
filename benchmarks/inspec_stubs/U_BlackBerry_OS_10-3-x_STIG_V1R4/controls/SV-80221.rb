control 'SV-80221' do
  title 'The BlackBerry MDM Agent must be configured to generate an audit record of required events: (See Vulnerability Discussion for list). This requirement only applies to Work space only and Work and personal - Regulated activation types and to version 10.3.3 and later of the BlackBerry OS.'
...
SFR ID: FAU_GEN.1.1(2) Refinement, MDM Agent EP'
  desc 'check', 'Review BlackBerry OS 10.3 configuration settings to determine if the BlackBerry is configured to generate an audit record of required events for Start-up and shutdown of the audit functions, Change in MDM policy, Device modification commanded by the MDM server, and Specifically defined auditable events in Table 7 of MDM Agent EP v.2.0. This procedure is performed on only on the BES console.
...
If the BES IT policy rule "Event logging" is not selected, this is a finding.
...
Note: Procedures above are for BES 12 only, and is not available on BES 10.'
  ...
  tag check_id: 'C-66387r2_chk'
  tag severity: 'low'
  tag gid: 'V-65731'
  tag rid: 'SV-80221r2_rule'
  tag stig_id: 'BB10-3X-020210'
  tag gtitle: 'PP-MDM-203001'
  tag fix_id: 'F-71775r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
