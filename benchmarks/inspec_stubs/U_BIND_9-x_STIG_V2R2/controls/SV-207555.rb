...
The slave should next issue its own NOTIFY announcements to the other authoritative name servers for the zone. The idea is that the primary master may not be able to notify all of the slave name servers for the zone itself, since it's possible some slaves can't communicate directly with the primary master (they use another slave as their master). Older BIND 8 slaves don't send NOTIFY messages unless explicitly configured to do so."
  desc 'check', 'If this is a secondary name server, this is Not Applicable.

On a master name server, verify that the global notify is disabled. The global entry for the name server is under the “Options” section and notify should be disabled at this section.
...
Restart the BIND 9.x process'
  ...
  tag check_id: 'C-7810r283719_chk'
  tag severity: 'low'
  tag gid: 'V-207555'
  tag rid: 'SV-207555r612253_rule'
  tag stig_id: 'BIND-9X-001057'
  tag gtitle: 'SRG-APP-000516-DNS-000088'
  tag fix_id: 'F-7810r283720_fix'
  tag 'documentable'
  tag legacy: ['SV-87033', 'V-72409']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
