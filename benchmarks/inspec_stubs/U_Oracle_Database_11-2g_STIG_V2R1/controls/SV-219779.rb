...
To test for timeout, open a connection and leave it idle for a period greater than the defined idle timeout setting enforced by the system.  Then try to use the connection.  If the connection is no longer active, then the mechanism deployed to terminate the connection is active and working.'
  desc 'fix', %q(Configure DBMS settings to terminate sessions upon user logout.  Configure DBMS settings to terminate sessions upon the occurrence of any organization or policy-defined session termination event.
- - - - -
...
To limit consumption of Oracle resources by unauthorized users and enable an audit trail, you should set time-limit values for the sqlnet.inbound_connect_timeout parameter in wall-clock seconds. (This parameter does not have default values.)  Failure resulting from sqlnet.inbound_connect_timeout will throw an ORA-03136 inbound connection timed out error.)
  ...
  tag check_id: 'C-21504r307186_chk'
  tag severity: 'medium'
  tag gid: 'V-219779'
  tag rid: 'SV-219779r397729_rule'
  tag stig_id: 'O112-C2-017600'
  tag gtitle: 'SRG-APP-000220-DB-000149'
  tag fix_id: 'F-21503r307187_fix'
  tag 'documentable'
  tag legacy: ['SV-66351', 'V-52135']
  tag cci: ['CCI-001185']
  tag nist: ['SC-23 (1)']
end
