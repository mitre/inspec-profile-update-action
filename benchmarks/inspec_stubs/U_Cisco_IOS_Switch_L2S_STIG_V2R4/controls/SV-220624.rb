control 'SV-220624' do
  ...
  desc 'VTP provides central management of VLAN domains, thus reducing administration in a switched network. When configuring a new VLAN on a VTP server, the VLAN is distributed through all switches in the domain. This reduces the need to configure the same VLAN everywhere. 
...
An attack can force a digest change for the VTP domain, enabling a rogue device to become the VTP server. This could allow unauthorized access to previously blocked VLANs or allow the addition of unauthorized switches into the domain. Authenticating VTP messages with a cryptographic hash function can reduce the risk of the VTP domain being compromised.'
  desc 'check', 'Review the switch configuration to verify that VTP is enabled using the show vtp status command as shown in the example below:
...
If VTP is enabled on the switch and is not authenticating VTP messages with a hash function using a configured password, this is a finding.'
  desc 'fix', 'Configure the switch to authenticate all VTP messages with a hash function using a configured password as shown in the example below:

SW1(config)#vtp password xxxxxxxxx'
  impact 0.5
  ref 'DPMS Target Cisco IOS Switch L2S'
  tag check_id: 'C-22339r507918_chk'
  tag severity: 'medium'
  tag gid: 'V-220624'
  tag rid: 'SV-220624r539671_rule'
  tag stig_id: 'CISC-L2-000030'
  tag gtitle: 'SRG-NET-000168-L2S-000019'
  tag fix_id: 'F-22328r507919_fix'
  tag 'documentable'
  tag legacy: ['SV-110219', 'V-101115']
  tag cci: ['CCI-000803']
  tag nist: ['IA-7']
end
