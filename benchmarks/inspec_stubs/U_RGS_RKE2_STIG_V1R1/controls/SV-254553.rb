...
kubectl get po -n kube-system -l component=kube-controller-manager -o=jsonpath="{.items[*].spec.containers[*].command}"

kubectl get po -n kube-system -l component=kube-scheduler -o=jsonpath="{.items[*].spec.containers[*].command}" 

kubectl get po -n kube-system -l component=kube-apiserver -o=jsonpath="{.items[*].spec.containers[*].command}"

For each, look for the existence of tls-min-version (use this command for an aid "ps -Af | grep tls-min-version"): 
...
systemctl restart rke2-server'
  ...
  tag check_id: 'C-58037r870262_chk'
  tag severity: 'high'
  tag gid: 'V-254553'
  tag rid: 'SV-254553r870263_rule'
