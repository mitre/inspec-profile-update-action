control 'SV-237101' do
  ...
  desc 'check', 'From the vSphere Web Client select the Virtual Machine, right click and go to Edit Settings >> VM Options Tab >> Advanced >> Configuration Parameters >> Edit Configuration. Find the "tools.guest.desktop.autolock" value and verify that it is set to "true".
...
If the VM is not Windows-based, this is not a finding.'
  desc 'fix', 'From the vSphere Client select the Virtual Machine, right click and go to Edit Settings >> VM Options Tab >> Advanced >> Configuration Parameters >> Edit Configuration. Find or create the "tools.guest.desktop.autolock" value and set it to "true".
...
Get-VM "VM Name" | Get-AdvancedSetting -Name tools.guest.desktop.autolock | Set-AdvancedSetting -Value true'
  ...
  tag check_id: 'C-40320r640138_chk'
  tag severity: 'medium'
  tag gid: 'V-237101'
  tag rid: 'SV-237101r640140_rule'
  tag stig_id: 'VMCH-65-000047'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-40283r640139_fix'
  tag 'documentable'
  tag legacy: ['SV-104477', 'V-94647']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
