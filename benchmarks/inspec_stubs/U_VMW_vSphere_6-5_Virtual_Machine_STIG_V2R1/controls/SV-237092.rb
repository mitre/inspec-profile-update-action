control 'SV-237092' do
  ...
  desc 'check', 'From the vSphere Web Client right-click the Virtual Machine and go to Edit Settings >> VM Options >> Advanced >> Configuration Parameters >> Edit Configuration. Verify the RemoteDisplay.maxConnections value is set to 1.
...
If the virtual machine advanced setting RemoteDisplay.maxConnections does not exist or is not set to 1, this is a finding.'
  desc 'fix', 'From the vSphere Web Client right-click the Virtual Machine and go to Edit Settings >> VM Options >> Advanced >> Configuration Parameters >> Edit Configuration. Find the RemoteDisplay.maxConnections value and set it to 1. If the setting does not exist, add the Name and Value setting at the bottom of screen.
...
Get-VM "VM Name" | Get-AdvancedSetting -Name RemoteDisplay.maxConnections | Set-AdvancedSetting -Value 1'
  ...
  tag check_id: 'C-40311r640111_chk'
  tag severity: 'medium'
  tag gid: 'V-237092'
  tag rid: 'SV-237092r640113_rule'
  tag stig_id: 'VMCH-65-000033'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-40274r640112_fix'
  tag 'documentable'
  tag legacy: ['SV-104453', 'V-94623']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
