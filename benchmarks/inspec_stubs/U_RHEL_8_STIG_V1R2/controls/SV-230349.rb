...
'
  desc 'check', 'Verify the operating system shell initialization file is configured to start each shell with the tmux terminal multiplexer with the following command:
...
If "tmux" is not configured as the example above, is commented out, or missing from the "/etc/bashrc" initialization file, this is a finding.'
  desc 'fix', 'Configure the operating system to initialize the tmux terminal multiplexer as each shell is called by adding the following line to the end of the "/etc/bashrc" configuration file:
...
This setting will take effect at next logon.'
  ...
  tag check_id: 'C-33018r567793_chk'
  tag severity: 'medium'
  tag gid: 'V-230349'
