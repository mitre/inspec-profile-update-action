...
Device authentication is a solution enabling an organization to manage devices. It is an additional layer of authentication ensuring that only specific preauthorized devices can access the system.'
  desc 'check', 'Verify that the switch configuration has 802.1x authentication implemented for all access switch ports connecting to LAN outlets (i.e., RJ-45 wall plates) or devices not located in the telecom room, wiring closets, or equipment rooms.
...
If 802.1x authentication or MAB is not configured on all access switch ports connecting to LAN outlets or devices not located in the telecom room, wiring closets, or equipment rooms, this is a finding.'
  desc 'fix', 'Configure 802.1 x authentications on all host-facing access switch ports. To authenticate devices that do not support 802.1x, MAB must be configured.
...
Note: Single-host is the default. Host-mode multi-domain (for VoIP phone plus PC) or multi-auth (multiple PCs connected to a hub) can be configured as alternatives.'
  ...
  tag check_id: 'C-22343r507930_chk'
  tag severity: 'medium'
  tag gid: 'V-220628'
  tag rid: 'SV-220628r539671_rule'
  tag stig_id: 'CISC-L2-000080'
  tag gtitle: 'SRG-NET-000343-L2S-000016'
  tag fix_id: 'F-22332r507931_fix'
  tag 'documentable'
  tag legacy: ['SV-110227', 'V-101123']
  tag cci: ['CCI-001958']
  tag nist: ['IA-3']
end
