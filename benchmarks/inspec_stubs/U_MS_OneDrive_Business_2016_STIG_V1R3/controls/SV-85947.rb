control 'SV-85947' do
  ...
  desc 'This setting will prevent users from going to onedrive.com to use the remote fetch feature to browse the file system of the machine (a feature supported on OneDrive running on Windows with the exception of Windows 8.1). This setting is for machines running 32-bit versions of Windows.'
...
Criteria: If the value GPOEnabled is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> OneDrive -> "Prevent users from using the remote fetch feature to access files on the machine (32-bit)" to "Enabled".'
  impact 0.5
  ref 'DPMS Target Microsoft OneDrive 2016'
  tag check_id: 'C-71721r7_chk'
  tag severity: 'medium'
  tag gid: 'V-71323'
  tag rid: 'SV-85947r2_rule'
  tag stig_id: 'DTOO602'
  tag gtitle: 'SRG-APP-000516'
  tag fix_id: 'F-77631r3_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
