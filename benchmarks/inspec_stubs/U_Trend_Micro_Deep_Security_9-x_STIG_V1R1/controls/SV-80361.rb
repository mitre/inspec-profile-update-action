...
Applications providing information flow control must be able to enforce approved authorizations for controlling the flow of information between interconnected systems in accordance with applicable policy.'
  desc 'check', %q(Review the Trend Deep Security server to ensure approved authorizations for controlling the flow of information between interconnected systems based on organization-defined information flow control policies are enforced.
...
If the information gathered does not match the settings within the application this is a finding.)
  desc 'fix', 'Configure the Trend Deep Security server to enforce approved authorizations for controlling the flow of information between interconnected systems based on organization-defined information flow control policies.
...
Select a Role and click Properties >> Computer Rights'
  ...
  tag check_id: 'C-66519r1_chk'
  tag severity: 'medium'
  tag gid: 'V-65871'
  tag rid: 'SV-80361r1_rule'
  tag stig_id: 'TMDS-00-000045'
  tag gtitle: 'SRG-APP-000039'
  tag fix_id: 'F-71947r1_fix'
  tag 'documentable'
  tag cci: ['CCI-001414']
  tag nist: ['AC-4']
end
