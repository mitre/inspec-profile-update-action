control 'SV-253714' do
  title 'MariaDB must check the validity of all data inputs except those specifically identified by the organization.'
...
This calls for inspection of application source code, which will require collaboration with the application developers. It is recognized that in many cases, the database administrator (DBA) is organizationally separate from the application developers, and may have limited, if any, access to source code. Nevertheless, protections of this type are so important to the secure operation of databases that they must not be ignored. At a minimum, the DBA must attempt to obtain assurances from the development organization that this issue has been addressed and must document what has been discovered."
  desc 'check', 'Review MariaDB code (trigger procedures, functions), application code, settings, column and field definitions, and constraints to determine whether the database is protected against invalid input.
...
Where a column/field is clearly identified by name, caption or context as Notes, Comments, Description, Text, etc., the absence of these protections is not a finding. Check application code that interacts with MariaDB for the use of prepared statements. If prepared statements are not used, this is a finding.'
...
Do not allow general users direct console access to MariaDB.'
  ...
  tag check_id: 'C-57166r841665_chk'
  tag severity: 'medium'
  tag gid: 'V-253714'
  tag rid: 'SV-253714r841667_rule'
  tag stig_id: 'MADB-10-005700'
  tag gtitle: 'SRG-APP-000251-DB-000160'
  tag fix_id: 'F-57117r841666_fix'
  tag 'documentable'
  tag cci: ['CCI-001310']
  tag nist: ['SI-10']
end
