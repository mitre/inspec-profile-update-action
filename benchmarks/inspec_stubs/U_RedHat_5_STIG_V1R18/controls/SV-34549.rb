control 'SV-34549' do
  ...
  desc 'check', 'Verify a system integrity baseline exists. The Advanced Intrusion Detection Environment (AIDE) is included in the distribution of RHEL. Other host intrusion detection system (HIDS) software is available but must be checked manually.
...
Configure the /etc/aide.conf file to ensure some form of cryptographic hash (e.g., md5,rmd160,sha256) is used for files. In the default /etc/aide.conf the "NORMAL" or "LSPP" rules which are used for virtually all files DO include some form of cryptographic hash.'
  ...
  tag check_id: 'C-37553r4_chk'
  tag severity: 'medium'
  tag gid: 'V-27250'
  tag rid: 'SV-34549r3_rule'
  tag stig_id: 'GEN000140-2'
  tag gtitle: 'GEN000140-2'
  tag fix_id: 'F-32799r4_fix'
  tag 'documentable'
