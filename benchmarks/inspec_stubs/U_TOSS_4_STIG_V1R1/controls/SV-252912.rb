control 'SV-252912' do
  title 'TOSS, for PKI-based authentication, must validate certificates by constructing a certification path (which includes status information) to an accepted trust anchor.'
...
'
  desc 'check', 'Verify TOSS for PKI-based authentication has valid certificates by constructing a certification path (which includes status information) to an accepted trust anchor.
...
If the root ca file is not a DoD-issued certificate with a valid date and installed in the /etc/sssd/pki/sssd_auth_ca_db.pem location, this is a finding.'
  desc 'fix', 'If PKI-based authentication is used, configure TOSS to validate certificates by constructing a certification path (which includes status information) to an accepted trust anchor.
...
/etc/sssd/pki/sssd_auth_ca_db.pem'
  ...
  tag check_id: 'C-56365r824058_chk'
  tag severity: 'medium'
  tag gid: 'V-252912'
  tag rid: 'SV-252912r824060_rule'
  tag stig_id: 'TOSS-04-010010'
  tag gtitle: 'SRG-OS-000066-GPOS-00034'
  tag fix_id: 'F-56315r824059_fix'
