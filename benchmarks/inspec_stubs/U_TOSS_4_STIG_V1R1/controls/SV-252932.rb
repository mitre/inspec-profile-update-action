control 'SV-252932' do
  title 'TOSS must have the packages required for multifactor authentication installed.'
...
This requirement only applies to components where this is specific to the function of the device or has the concept of an organizational user (e.g., VPN, proxy capability). This does not apply to authentication for the purpose of configuring the device itself (management).'
  desc 'check', 'Verify TOSS has the packages required for multifactor authentication installed with the following commands:
...
If the "openssl-pkcs11" package is not installed, ask the administrator to indicate what type of multifactor authentication is being utilized and what packages are installed to support it. If there is no evidence of multifactor authentication being used, this is a finding.'
  desc 'fix', 'Configure TOSS to implement multifactor authentication by installing the required package with the following command:

$ sudo yum install openssl-pkcs11'
  impact 0.5
  ref 'DPMS Target TOSS 4'
  tag check_id: 'C-56385r824118_chk'
  tag severity: 'medium'
  tag gid: 'V-252932'
  tag rid: 'SV-252932r824120_rule'
  tag stig_id: 'TOSS-04-010240'
  tag gtitle: 'SRG-OS-000375-GPOS-00160'
  tag fix_id: 'F-56335r824119_fix'
  tag 'documentable'
  tag cci: ['CCI-001948']
  tag nist: ['IA-2 (11)']
end
