control 'SV-252936' do
  title 'The debug-shell systemd service must be disabled on TOSS.'
  desc 'The debug-shell requires no authentication and provides root privileges to anyone who has physical access to the machine. While this feature is disabled by default, masking it adds an additional layer of assurance that it will not be enabled via a dependency in systemd. This also prevents attackers with physical access from trivially bypassing security on the machine through valid troubleshooting configurations and gaining root access when the system is rebooted.'
  desc 'check', 'Verify TOSS is configured to mask the debug-shell systemd service with the following command:

$ sudo systemctl status debug-shell.service

debug-shell.service
Loaded: masked (Reason: Unit debug-shell.service is masked.)
Active: inactive (dead)

If the "debug-shell.service" is loaded and not masked, this is a finding.'
  desc 'fix', 'Configure the system to mask the debug-shell systemd service with the following command:

$ sudo systemctl mask debug-shell.service

Created symlink /etc/systemd/system/debug-shell.service -> /dev/null

Reload the daemon to take effect.

$ sudo systemctl daemon-reload'
  impact 0.5
  ref 'DPMS Target TOSS 4'
  tag check_id: 'C-56389r824130_chk'
  tag severity: 'medium'
  tag gid: 'V-252936'
  tag rid: 'SV-252936r824132_rule'
  tag stig_id: 'TOSS-04-010340'
  tag gtitle: 'SRG-OS-000480-GPOS-00227'
  tag fix_id: 'F-56339r824131_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
