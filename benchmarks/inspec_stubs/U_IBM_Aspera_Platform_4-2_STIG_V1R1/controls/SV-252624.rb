control 'SV-252624' do
  title 'The IBM Aspera High-Speed Transfer Endpoint must restrict users from using transfer services by default.'
...
The IBM Aspera High Speed Transfer Endpoint inherently uses file and group ownership of files and directories to support authorization for all supported operating systems. As an additional step and security best practice, ensure all transfers in or out of the authenticated connection are configured to be controlled based on privileges granted to specific users and groups within IBM Aspera configuration.'
  desc 'check', 'Verify the Aspera High-Speed Transfer Endpoint restricts users from using transfer services by default with the following commands:
...
If the results produce an "allow" value, this is a finding.'
  desc 'fix', 'Configure the Aspera High-Speed Transfer Endpoint to restrict users from using transfer services by default with the following commands:
...
$ sudo systemctl restart asperanoded.service'
  ...
  tag check_id: 'C-56080r818040_chk'
  tag severity: 'medium'
  tag gid: 'V-252624'
  tag rid: 'SV-252624r818042_rule'
  tag stig_id: 'ASP4-TE-030220'
  tag gtitle: 'SRG-NET-000015-ALG-000016'
  tag fix_id: 'F-56030r818041_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
