control 'SV-252596' do
  title 'The IBM Aspera Faspex Server must restrict users read, write, and browse permissions by default.'
...
IBM Aspera High Speed Transfer Server and IBM Aspera High Speed Transfer Endpoint inherently use file and group ownership of files and directories to support authorization for all supported operating systems. As an additional step and security best practice, ensure all transfers in or out of the authenticated connection are configured to be controlled based on privileges granted to specific users and groups within IBM Aspera configuration.'
  desc 'check', %q(If the IBM Aspera Faspex feature of the Aspera Platform is not installed, this is Not Applicable.
...
If no results are returned or if the results produce a "true" value, this is a finding.)
  desc 'fix', %q(Configure the IBM Aspera Faspex to restrict users' read, write, and browse permissions by default with the following command:
...
$ sudo systemctl restart asperanoded.service)
  ...
  tag check_id: 'C-56052r817956_chk'
  tag severity: 'medium'
  tag gid: 'V-252596'
  tag rid: 'SV-252596r817958_rule'
  tag stig_id: 'ASP4-FA-050320'
  tag gtitle: 'SRG-NET-000015-ALG-000016'
  tag fix_id: 'F-56002r817957_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
