control 'SV-252617' do
  title 'The IBM Aspera High-Speed Transfer Endpoint must enable content protection for each transfer user by encrypting passphrases used for server-side encryption at rest (SSEAR).'
...
The askmscli tool sets content-protection secrets only for each user, not for groups and not for all users on a node. Each transfer user requires their own content-protection secret for SSEAR.'
  desc 'check', 'Verify the IBM High-Speed Transfer Endpoint enables content protection for each transfer user by encrypting passphrases used for SSEAR with the following command:
...
If the command returns "No records found for ssear", this is a finding.'
  desc 'fix', 'Configure the IBM High-Speed Transfer Endpoint to enable content protection for each transfer user by encrypting passphrases used for SSEAR with the following command:

$ sudo /opt/aspera/bin/askmscli -u <transferuser> -s ssear'
  impact 0.5
  ref 'DPMS Target IBM Aspera Platform 4.2'
  tag check_id: 'C-56073r818019_chk'
  tag severity: 'medium'
  tag gid: 'V-252617'
  tag rid: 'SV-252617r818021_rule'
  tag stig_id: 'ASP4-TE-030150'
  tag gtitle: 'SRG-NET-000512-ALG-000062'
  tag fix_id: 'F-56023r818020_fix'
  tag 'documentable'
