control 'SV-252635' do
  title 'The IBM Aspera High-Speed Transfer Server must have a master-key set to encrypt the dynamic token encryption key.'
...
'
  desc 'check', 'Verify the IBM High-Speed Transfer Server has a master-key set to encrypt the dynamic token encryption key with the following commands:
...
If either command returns "No records found for Redis-master-key", this is a finding.'
  desc 'fix', %q(Configure the IBM High-Speed Transfer Server to set a master-key to encrypt the dynamic token encryption key with the following command:
...
$ sudo systemctl restart asperanoded.service)
  ...
  tag check_id: 'C-56091r818073_chk'
  tag severity: 'medium'
  tag gid: 'V-252635'
  tag rid: 'SV-252635r818075_rule'
  tag stig_id: 'ASP4-TS-020190'
  tag gtitle: 'SRG-NET-000063-ALG-000012'
  tag fix_id: 'F-56041r818074_fix'
