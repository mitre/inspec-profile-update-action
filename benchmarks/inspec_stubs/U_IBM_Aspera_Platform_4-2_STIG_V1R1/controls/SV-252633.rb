control 'SV-252633' do
  title 'The IBM Aspera High-Speed Transfer Server must enable password protection of the node database.'
...
System administrators can set a secure password for clients to authenticate with a Redis database. When the authorization layer is enabled, Redis refuses any query by unauthenticated clients. A client can authenticate itself by sending the AUTH command followed by the password.'
  desc 'check', 'Verify the IBM High-Speed Transfer Server enables password protection of the node database with the following commands:
...
If the command results do not state "Authentication required", this is a finding.'
  desc 'fix', 'Configure the IBM High-Speed Transfer Server to enable password protection of the node database.
...
$ sudo /opt/aspera/bin/askmscli -i -u asperadaemon'
  ...
  tag check_id: 'C-56089r818067_chk'
  tag severity: 'medium'
  tag gid: 'V-252633'
  tag rid: 'SV-252633r818069_rule'
  tag stig_id: 'ASP4-TS-020170'
  tag gtitle: 'SRG-NET-000015-ALG-000016'
  tag fix_id: 'F-56039r818068_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
