control 'SV-257540' do
  title 'OpenShift must disable root and terminate network connections.'
  desc 'Direct login as the "root" user must be disabled to prevent unrestricted access and control over the entire system. 
...
'
  desc 'check', %q(Verify SSH is restricted from logging on as root and network connections are terminated.
...
If "ClientAliveInterval" does not exist, or has a value of > 600 in "/etc/ssh/sshd_config", or is commented out, this is a finding.)
  desc 'fix', 'Apply the machine config that disables root and terminates network connections by executing the following:
...
" | oc apply -f -
  ...
  tag check_id: 'C-61275r921561_chk'
  tag severity: 'high'
  tag gid: 'V-257540'
  tag rid: 'SV-257540r921563_rule'
  tag stig_id: 'CNTR-OS-000400'
  tag gtitle: 'SRG-APP-000148-CTR-000335'
  tag fix_id: 'F-61199r921562_fix'
  tag satisfies: ['SRG-APP-000148-CTR-000335', 'SRG-APP-000190-CTR-000500']
  tag 'documentable'
  tag cci: ['CCI-000764', 'CCI-001133']
  tag nist: ['IA-2', 'SC-10']
end
