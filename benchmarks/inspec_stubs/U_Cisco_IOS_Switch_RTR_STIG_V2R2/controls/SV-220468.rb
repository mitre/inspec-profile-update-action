...
This requirement applies to all IPv4 and IPv6 protocols that are used to exchange routing or packet forwarding information; this includes all Interior Gateway Protocols (such as OSPF, EIGRP, and IS-IS) and Exterior Gateway Protocols (such as BGP), MPLS-related protocols (such as LDP), and multicast-related protocols.)
  desc 'check', 'Review the switch configuration. Verify that authentication is enabled for all routing protocols. The configuration examples below depict OSPF and EIGRP authentication. 
...
If authentication is not enabled on all routing protocols, this is a finding.'
  desc 'fix', 'Configure authentication to be enabled for every protocol that affects the routing or forwarding tables. The example configuration commands below enables OSPF and EIGRP authentication. 
...
SW1(config-if)#end'
  ...
  tag check_id: 'C-22183r508477_chk'
  tag severity: 'medium'
  tag gid: 'V-220468'
  tag rid: 'SV-220468r856248_rule'
  tag stig_id: 'CISC-RT-000020'
  tag gtitle: 'SRG-NET-000230-RTR-000001'
  tag fix_id: 'F-22172r508478_fix'
  tag 'documentable'
  tag legacy: ['SV-110687', 'V-101583']
