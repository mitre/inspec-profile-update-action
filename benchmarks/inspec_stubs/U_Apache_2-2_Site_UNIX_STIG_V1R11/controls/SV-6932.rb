...
#!/usr/local/bin/perl –T'
  desc 'check', "When a PERL script is invoked for execution on a UNIX server, the method which invokes the script must utilize the TAINT option.
...
NOTE: If the mod_perl module is installed and the directive “PerlTaintCheck on” in the httpd.conf is used, this satisfies the requirement."
  desc 'fix', 'Add the TAINT call to the PERL script.

#!/usr/local/bin/perl –T'
  impact 0.5
  ref 'DPMS Target Apache Site 2.x'
  tag check_id: 'C-30932r1_chk'
  tag severity: 'medium'
  tag gid: 'V-2272'
