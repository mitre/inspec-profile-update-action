control 'SV-220877' do
  title 'Windows 10 Exploit Protection system-level mitigation, Validate heap integrity, must be on.'
  desc 'Exploit protection in Windows 10 enables mitigations against potential threats at the system and application level.  Several mitigations, including "Validate heap integrity", are enabled by default at the system level. "Validate heap integrity" terminates a process when heap corruption is detected. If this is turned off, Windows 10 may be subject to various exploits.'
...
The XML file is applied with the group policy setting Computer Configuration >> Administrative Settings >> Windows Components >> Windows Defender Exploit Guard >> Exploit Protection >> "Use a common set of exploit protection settings" configured to "Enabled" with file name and location defined under "Options:". It is recommended the file be in a read-only network location.'
  ...
  tag check_id: 'C-22592r555116_chk'
  tag severity: 'medium'
  tag gid: 'V-220877'
  tag rid: 'SV-220877r569187_rule'
  tag stig_id: 'WN10-EP-000060'
  tag gtitle: 'SRG-OS-000480-GPOS-00227'
  tag fix_id: 'F-22581r555117_fix'
  tag 'documentable'
  tag legacy: ['V-77103', 'SV-91799']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
