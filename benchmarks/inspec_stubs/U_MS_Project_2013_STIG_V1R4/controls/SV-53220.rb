...
This functionality can be controlled separately for instances of Internet Explorer spawned by Office applications (for example, if a user clicks a link in an Office document or selects a menu option that loads a web page). If user names and passwords in URLs are allowed, users could be diverted to dangerous web pages, which would pose a security risk.'
  desc 'check', %q(Verify the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Disable user name and password" is "Enabled" and a check in the 'winproj.exe' check box is present.
...
Criteria: If the value winproj.exe is REG_DWORD = 1, this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Disable user name and password" to "Enabled" and place a check in the 'winproj.exe' check box.)
  impact 0.5
  ref 'DPMS Target Microsoft Project 2013'
  tag check_id: 'C-47527r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17173'
  tag rid: 'SV-53220r1_rule'
  tag stig_id: 'DTOO104'
  tag gtitle: 'DTOO104 - Disable user name and password'
  tag fix_id: 'F-46147r1_fix'
  tag 'documentable'
