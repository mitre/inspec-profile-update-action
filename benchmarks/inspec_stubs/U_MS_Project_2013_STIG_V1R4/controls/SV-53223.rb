control 'SV-53223' do
  ...
  desc 'check', %q(Verify the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Navigate URL" is "Enabled" and a check in the 'winproj.exe' check box is present.
...
Criteria: If the value winproj.exe is REG_DWORD = 1, this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Navigate URL" to "Enabled" and place a check in the 'winproj.exe' check box.)
  impact 0.5
  ref 'DPMS Target Microsoft Project 2013'
  tag check_id: 'C-47530r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17183'
  tag rid: 'SV-53223r1_rule'
  tag stig_id: 'DTOO123'
  tag gtitle: 'DTOO123-Block Navigation to URL from Office'
  tag fix_id: 'F-46150r1_fix'
  tag 'documentable'
