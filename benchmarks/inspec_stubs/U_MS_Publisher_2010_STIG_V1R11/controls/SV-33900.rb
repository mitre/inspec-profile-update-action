control 'SV-33900' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Navigate URL” must be “Enabled” and a check in the ‘mspub.exe’ check box must be present.
...
Criteria: If the value mspub.exe is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2010 (Machine) -> Security Settings -> IE Security “Navigate URL” to “Enabled” and place a check in the ‘mspub.exe’ check box.'
  impact 0.5
  ref 'DPMS Target Microsoft Publisher 2010'
  tag check_id: 'C-34496r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17183'
  tag rid: 'SV-33900r1_rule'
  tag stig_id: 'DTOO123 - Publisher'
  tag gtitle: 'DTOO123-Block Navigation to URL from Office'
  tag fix_id: 'F-30024r1_fix'
  tag 'documentable'
