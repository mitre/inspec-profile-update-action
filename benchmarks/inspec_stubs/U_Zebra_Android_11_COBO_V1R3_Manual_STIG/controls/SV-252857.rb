control 'SV-252857' do
  title 'Zebra Android 11 allow list must be configured to not include applications with the following characteristics: 
...
SFR ID: FMT_SMF_EXT.1.1 #8b'
  desc 'check', 'Review Zebra Android device configuration settings to determine if the mobile device has an application allow list configured and that the application allow list does not include applications with the following characteristics:
...
If the EMM console device policy includes applications with unauthorized characteristics, this is a finding.'
  desc 'fix', 'Configure the Zebra Android 11 device application allow list to exclude applications with the following characteristics:
...
2. Before selecting an app, review the app details and privacy policy to ensure the app does not include prohibited characteristics.'
  ...
  tag check_id: 'C-56313r820496_chk'
  tag severity: 'medium'
  tag gid: 'V-252857'
  tag rid: 'SV-252857r820498_rule'
  tag stig_id: 'ZEBR-11-001100'
  tag gtitle: 'PP-MDF-301100'
  tag fix_id: 'F-56263r820497_fix'
  tag 'documentable'
