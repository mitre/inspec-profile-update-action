control 'SV-252855' do
  title 'Zebra Android 11 must be configured to enforce an application installation policy by specifying one or more authorized application repositories, including [selection: DoD-approved commercial app repository, EMM server, mobile application store].'
...
SFR ID: FMT_SMF_EXT.1.1 #8a'
  desc 'check', 'Review Zebra Android device configuration settings to determine if the mobile device has only approved application repositories (DoD-approved commercial app repository, EMM server, and/or mobile application store).
...
If the EMM console device policy is not set to allow connections to "Only approved application repositories" or on the Android 11 device, the device policy is not set to allow connections to "Only approved application repositories, this is a finding.'
  desc 'fix', 'Configure the Zebra Android 11 device to disable unauthorized application repositories.
...
3. Toggle "Disallow installs from unknown sources globally" to "On".'
  ...
  tag check_id: 'C-56311r820490_chk'
  tag severity: 'medium'
  tag gid: 'V-252855'
  tag rid: 'SV-252855r820492_rule'
  tag stig_id: 'ZEBR-11-000800'
  tag gtitle: 'PP-MDF-301080'
  tag fix_id: 'F-56261r820491_fix'
  tag 'documentable'
