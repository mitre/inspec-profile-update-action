control 'SV-53328' do
  ...
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft InfoPath 2013 -> Security -> Trust Center "Require that application add-ins are signed by Trusted Publisher" must be set to "Enabled".
...
Criteria: If the value RequireAddinSig is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft InfoPath 2013 -> Security -> Trust Center "Require that application add-ins are signed by Trusted Publisher" to "Enabled".'
  impact 0.5
  ref 'DPMS Target Microsoft InfoPath 2013'
  tag check_id: 'C-47610r1_chk'
  tag severity: 'medium'
  tag gid: 'V-26589'
  tag rid: 'SV-53328r1_rule'
  tag stig_id: 'DTOO127'
  tag gtitle: 'DTOO127 - Add-ins are signed by Trusted Publisher'
  tag fix_id: 'F-46258r1_fix'
  tag 'documentable'
