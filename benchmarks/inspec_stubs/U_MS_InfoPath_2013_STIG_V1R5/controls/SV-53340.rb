control 'SV-53340' do
  ...
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft InfoPath 2013 -> Security -> "Disable opening of solutions from the Internet security zone" must be set to "Enabled".
...
Criteria: If the value AllowInternetSolutions is REG_DWORD = 0, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft InfoPath 2013 -> Security -> "Disable opening of solutions from the Internet security zone" to "Enabled".'
  impact 0.5
  ref 'DPMS Target Microsoft InfoPath 2013'
  tag check_id: 'C-47615r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17663'
  tag rid: 'SV-53340r1_rule'
  tag stig_id: 'DTOO158'
  tag gtitle: 'DTOO158 - Solutions from the Internet Zone'
  tag fix_id: 'F-46268r1_fix'
  tag 'documentable'
