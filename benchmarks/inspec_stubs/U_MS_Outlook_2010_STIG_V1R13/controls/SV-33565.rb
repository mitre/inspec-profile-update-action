...
By default, Outlook does not run in FIPS-compliant mode. Organizations that do business with the U.S. government but do not run Outlook in FIPS-compliant mode risk violating the government's rules regarding the handling of sensitive information."
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft Outlook 2010 -> Security -> Cryptography “Run in FIPS compliant mode” must be set to “Enabled”.
...
Criteria: If the value FIPSMode is REG_DWORD = 1, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Outlook 2010 -> Security -> Cryptography “Run in FIPS compliant mode” to “Enabled”.'
  impact 0.5
  ref 'DPMS Target Microsoft Outlook 2010'
  tag check_id: 'C-34025r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17787'
  tag rid: 'SV-33565r1_rule'
  tag stig_id: 'DTOO262 - Outlook'
  tag gtitle: 'DTOO262 - FIPS compliant mode'
  tag fix_id: 'F-29710r1_fix'
  tag 'documentable'
