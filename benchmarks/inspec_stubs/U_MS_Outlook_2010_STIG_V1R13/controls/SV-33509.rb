...
By default, Outlook does not allow users to define folder home pages for folders in non-default stores. If this configuration is changed, users can create and access dangerous folder home pages for Outlook data files (.pst) and other non-default stores, which can compromise the security of the users' data."
  desc 'check', 'The policy value for User Configuration -> Administrative Templates -> Microsoft Outlook 2010 -> Outlook Options -> Other -> Advanced “Do not allow folders in non-default stores to be set as folder home pages” must be set to “Enabled”.
...
Criteria: If the value NonDefaultStoreScript is REG_DWORD = 0, this is not a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Outlook 2010 -> Outlook Options -> Other -> Advanced “Do not allow folders in non-default stores to be set as folder home pages” to “Enabled”.'
  impact 0.5
  ref 'DPMS Target Microsoft Outlook 2010'
  tag check_id: 'C-33995r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17674'
  tag rid: 'SV-33509r1_rule'
  tag stig_id: 'DTOO230 - Outlook'
  tag gtitle: 'DTOO230 - No fldr home pages / non-default stores'
  tag fix_id: 'F-29684r1_fix'
  tag 'documentable'
