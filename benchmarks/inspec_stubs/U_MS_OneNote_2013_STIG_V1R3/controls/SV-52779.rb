...
This functionality can be controlled separately for instances of Internet Explorer spawned by Office applications (for example, if a user clicks a link in an Office document or selects a menu option that loads a web page). A security risk could occur if potentially dangerous controls are allowed to load.'
  desc 'check', %q(Verify the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Bind to Object" is set to "Enabled" and a check in the 'onent.exe' check box is selected.
...
Criteria: If the value onenote.exe is REG_DWORD = 1, this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2013 (Machine) -> Security Settings -> IE Security "Bind to Object" to "Enabled" and place a check in the 'onent.exe' check box.)
  impact 0.5
  ref 'DPMS Target Microsoft OneNote 2013'
  tag check_id: 'C-47108r1_chk'
  tag severity: 'medium'
  tag gid: 'V-17174'
  tag rid: 'SV-52779r1_rule'
  tag stig_id: 'DTOO111'
  tag gtitle: 'DTOO111 - Enable IE Bind to Object'
  tag fix_id: 'F-45705r1_fix'
  tag 'documentable'
