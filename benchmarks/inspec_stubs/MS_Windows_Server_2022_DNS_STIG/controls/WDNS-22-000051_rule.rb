control 'WDNS-22-000051_rule' do
  title 'The Windows 2022 DNS Server must be configured with the Delegation Signer (DS) Resource Records (RR) carrying the signature for the RR that contains the public key of the child zone.'
...
From the expanded list, right-click to select the zone (repeat for each hosted zone), point to DNSSEC, and then click "Sign the Zone" using either approved saved parameters or approved custom parameters.'
  impact 0.5
  tag check_id: 'C-WDNS-22-000051_chk'
  tag severity: 'medium'
  tag gid: 'WDNS-22-000051'
  tag rid: 'WDNS-22-000051_rule'
  tag stig_id: 'WDNS-22-000051'
  tag gtitle: 'SRG-APP-000214-DNS-000025'
  tag fix_id: 'F-WDNS-22-000051_fix'
  tag 'documentable'
  tag cci: ['CCI-001179']
  tag nist: ['SC-20 b']
end
