control 'SV-235078' do
  title 'The Honeywell Mobility Edge Android Pie device must be configured to disable exceptions to the access control policy that prevents application processes from accessing all data stored by other application processes.'
...
SFR ID: FMT_SMF_EXT.1.1 #42, FDP_ACF_EXT.1.2'
  desc 'check', 'Review documentation on the Honeywell Android device and inspect the configuration on the Honeywell Android device to verify the access control policy that prevents [selection: application processes] from accessing [selection: all] data stored by other [selection: application processes] is enabled.
...
If the MDM console device policy is not set to disable data sharing between profiles, this is a finding.'
  desc 'fix', 'Configure the Honeywell Android Pie to enable the access control policy that prevents [selection: application processes, groups of application processes] from accessing [selection: all, private] data stored by other [selection: application processes, groups of application processes].
...
4. Select "Disallow sharing data into the profile".'
  ...
  tag check_id: 'C-38297r623249_chk'
  tag severity: 'medium'
  tag gid: 'V-235078'
  tag rid: 'SV-235078r626527_rule'
  tag stig_id: 'HONW-09-004500'
  tag gtitle: 'PP-MDF-301260'
  tag fix_id: 'F-38260r623250_fix'
  tag 'documentable'
