control 'SV-239466' do
  title 'The SLES for vRealize audit system must be configured to audit all attempts to alter system time through /etc/localtime.'
...
4) All kernel module load, unload, and restart actions.'
  desc 'check', 'To determine if SLES for vRealize is configured to audit attempts to alter time via the /etc/localtime file, run the following command: 
...
If no line is returned, this is a finding.'
  desc 'fix', "To configure the SLES for vRealize to audit attempts to alter time via the /etc/localtime file, run the following command:
...
# /etc/dodscript.sh"
  ...
  tag check_id: 'C-42699r661847_chk'
  tag severity: 'medium'
  tag gid: 'V-239466'
  tag rid: 'SV-239466r767699_rule'
  tag stig_id: 'VROM-SL-000200'
  tag gtitle: 'VROM-SL-000200'
  tag fix_id: 'F-42658r661848_fix'
  tag 'documentable'
  tag legacy: ['SV-99053', 'V-88403']
  tag cci: ['CCI-000169']
  tag nist: ['AU-12 a']
end
