...
To address access requirements, many SLES for vRealize operating systems may be integrated with enterprise-level authentication/access mechanisms that meet or exceed access control policy requirements.'
...
If any temporary accounts have no expiration date set or do not expire within "72" hours, this is a finding.'
  desc 'fix', 'In the event temporary accounts are required, configure the system to terminate them after "72" hours. For every temporary account, run the following command to set an expiration date on it, substituting "system_account_name" for the appropriate value.
...
`date -d "+3 days" +%Y-%m-%d` gets the expiration date for the account at the time of running the command.'
  ...
  tag check_id: 'C-42675r661775_chk'
  tag severity: 'medium'
  tag gid: 'V-239442'
  tag rid: 'SV-239442r661777_rule'
  tag stig_id: 'VROM-SL-000010'
  tag gtitle: 'SRG-OS-000002-GPOS-00002'
  tag fix_id: 'F-42634r661776_fix'
  tag 'documentable'
  tag legacy: ['SV-99005', 'V-88355']
  tag cci: ['CCI-000016']
  tag nist: ['AC-2 (2)']
end
