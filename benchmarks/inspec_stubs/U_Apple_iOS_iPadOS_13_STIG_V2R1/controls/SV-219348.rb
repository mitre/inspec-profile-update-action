control 'SV-219348' do
  title 'Apple iOS/iPadOS must be configured to not allow passwords that include more than two repeating or sequential characters.'
...
In the Apple iOS/iPadOS management tool, verify the "Allow simple value" is unchecked. Alternatively, verify the text "<key>allowSimple</key> <false/>" appears in the configuration profile (.mobileconfig file).
...
If "Allow simple value" is checked in the Apple iOS/iPadOS management tool, "<key>allowSimple</key> <true/>" appears in the Configuration Profile, or the password policy on the iPhone and iPad does not have "Simple passcodes allowed" set to "No", this is a finding.'
  ...
  tag check_id: 'C-21073r547561_chk'
  tag severity: 'medium'
  tag gid: 'V-219348'
  tag rid: 'SV-219348r604137_rule'
  tag stig_id: 'AIOS-13-000200'
  tag gtitle: 'PP-MDF-301020'
  tag fix_id: 'F-21072r547562_fix'
  tag 'documentable'
  tag legacy: ['SV-106525', 'V-97421']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
