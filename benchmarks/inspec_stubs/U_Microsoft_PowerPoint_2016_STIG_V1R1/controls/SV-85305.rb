control 'SV-85305' do
  ...
  desc 'check', %q(Verify the policy value for User Configuration -> Administrative Templates -> Microsoft PowerPoint 2016 -> PowerPoint Options -> Security -> Trust Center "VBA Macro Notification Settings" is set to "Enabled: Disable all with notification".  The options 'Enabled: Disable all except digitally signed macros' and 'Enabled: Disable all without notification' are more restrictive and also acceptable values.
...
Criteria: If the value VBAWarnings is REG_DWORD = 2, this is not a finding.   Values of REG_DWORD = 3 or 4 are also acceptable values.  If the registry key does not exist or the value is REG_DWORD = 1, then this is a finding.)
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft PowerPoint 2016 -> PowerPoint Options -> Security -> Trust Center "VBA Macro Notification Settings" to "Enabled (Disable all with notification)".'
  impact 0.5
  ref 'DPMS Target Microsoft PowerPoint 2016'
  tag check_id: 'C-71161r2_chk'
  tag severity: 'medium'
  tag gid: 'V-70681'
  tag rid: 'SV-85305r1_rule'
  tag stig_id: 'DTOO304'
  tag gtitle: 'SRG-APP-000141'
  tag fix_id: 'F-77003r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
end
