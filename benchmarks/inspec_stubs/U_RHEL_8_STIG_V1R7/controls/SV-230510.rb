...
If results are returned and the "noexec" option is missing, or if /dev/shm is mounted without the "noexec" option, this is a finding.'
  desc 'fix', 'Configure the system so that /dev/shm is mounted with the "noexec" option by adding /modifying the /etc/fstab with the following line:

tmpfs /dev/shm tmpfs defaults,nodev,nosuid,noexec 0 0'
  impact 0.5
  ref 'DPMS Target Red Hat Enterprise Linux 8'
  tag check_id: 'C-33179r568276_chk'
  tag severity: 'medium'
  tag gid: 'V-230510'
