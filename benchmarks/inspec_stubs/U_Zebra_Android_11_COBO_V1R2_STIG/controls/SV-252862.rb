control 'SV-252862' do
  title 'Zebra Android 11 must be configured to display the DoD advisory warning message at start-up or each time the user unlocks the device.'
  desc %q(The Zebra Android 11 is required to display the DoD-approved system use notification message or banner before granting access to the system that provides privacy and security notices consistent with applicable federal laws, Executive Orders, directives, policies, regulations, standards, and guidance. Required banners help ensure that DoD can audit and monitor the activities of mobile device users without legal restriction.
...
1. By placing the DoD warning banner text in the user agreement signed by each Zebra Android device user (preferred method).
2. By configuring the warning banner text on the EMM console and installing the banner on each managed mobile device.

Determine which method is used at the Zebra Android device site and follow the appropriate validation procedure below.

Validation Procedure for Method #1:
Review the signed user agreements for several Zebra Android device users and verify the agreement includes the required DoD warning banner text.
...
1. By placing the DoD warning banner text in the user agreement signed by each Zebra Android 11 device user (preferred method).
2. By configuring the warning banner text on the EMM console and installing the banner on each managed mobile device.
...
4. Enter in message.'
  ...
  tag check_id: 'C-56318r820511_chk'
  tag severity: 'low'
  tag gid: 'V-252862'
  tag rid: 'SV-252862r820513_rule'
  tag stig_id: 'ZEBR-11-003400'
  tag gtitle: 'PP-MDF-301200'
  tag fix_id: 'F-56268r820512_fix'
  tag 'documentable'
  tag cci: ['CCI-000048']
  tag nist: ['AC-8 a']
end
