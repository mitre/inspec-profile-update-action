...
This functionality can be controlled separately for instances of Internet Explorer spawned by Office applications (for example, if a user clicks a link in an Office document or selects a menu option that loads a web page). A security risk could occur if potentially dangerous controls are allowed to load.'
  desc 'check', %q(Verify the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2016 (Machine) -> Security Settings -> IE Security "Bind to Object" is set to "Enabled" and 'visio.exe' is checked.
...
Criteria: If the value visio.exe is REG_DWORD = 1, this is not a finding.)
  desc 'fix', %q(Set the policy value for Computer Configuration -> Administrative Templates -> Microsoft Office 2016 (Machine) -> Security Settings -> IE Security "Bind to Object" to "Enabled" and place a check in the 'visio.exe' check box.)
  impact 0.5
  ref 'DPMS Target Microsoft Visio 2016'
  tag check_id: 'C-71241r3_chk'
  tag severity: 'medium'
  tag gid: 'V-70805'
  tag rid: 'SV-85429r1_rule'
  tag stig_id: 'DTOO111'
  tag gtitle: 'SRG-APP-000112'
  tag fix_id: 'F-77131r1_fix'
  tag 'documentable'
  tag cci: ['CCI-001695']
  tag nist: ['SC-18 (3)']
end
