control 'SV-220623' do
  title 'The Cisco switch must uniquely identify and authenticate all network-connected endpoint devices before establishing any connection.'
  ...
  desc 'check', 'Verify that the switch configuration has 802.1x authentication implemented for all access switch ports connecting to LAN outlets (i.e., RJ-45 wall plates) or devices not located in the telecom room, wiring closets, or equipment rooms. MAC Authentication Bypass (MAB) must be configured on switch ports connected to devices that do not support an 802.1x supplicant.
...
If 802.1x authentication or MAB is not configured on all access switch ports connecting to LAN outlets or devices not located in the telecom room, wiring closets, or equipment rooms, this is a finding.'
  desc 'fix', 'Configure 802.1 x authentications on all host-facing access switch ports. To authenticate devices that do not support 802.1x, MAC Authentication Bypass must be configured.
...
Note: Single-host is the default. Host-mode multi-domain (for VoIP phone plus PC) or multi-auth (multiple PCs connected to a hub) can be configured as alternatives.'
  ...
  tag check_id: 'C-22338r507915_chk'
  tag severity: 'high'
  tag gid: 'V-220623'
  tag rid: 'SV-220623r863275_rule'
  tag stig_id: 'CISC-L2-000020'
  tag gtitle: 'SRG-NET-000148-L2S-000015'
  tag fix_id: 'F-22327r507916_fix'
  tag 'documentable'
  tag legacy: ['SV-110217', 'V-101113']
