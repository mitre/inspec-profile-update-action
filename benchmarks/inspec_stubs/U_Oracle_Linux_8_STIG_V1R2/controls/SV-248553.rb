control 'SV-248553' do
  title 'OL 8 must be configured so that all network connections associated with SSH traffic are terminated at the end of the session or after 10 minutes of inactivity.'
...
'
  desc 'check', 'Verify all network connections associated with SSH traffic are automatically terminated after 10 minutes of inactivity. 
 
Check that the "ClientAliveInterval" variable is set to a value of "600" or less by running the following command: 
 
$ sudo grep -i ClientAliveInterval /etc/ssh/sshd_config 
...
If "ClientAliveInterval" does not exist, does not have a product value of "600" or less in "/etc/ssh/sshd_config", or is commented out, this is a finding.'
  desc 'fix', 'Configure OL 8 to automatically terminate all network connections associated with SSH traffic after 10 minutes of inactivity. 
...
$ sudo systemctl restart sshd.service'
  ...
  tag check_id: 'C-51987r779223_chk'
  tag severity: 'medium'
  tag gid: 'V-248553'
  tag rid: 'SV-248553r779225_rule'
  tag stig_id: 'OL08-00-010201'
  tag gtitle: 'SRG-OS-000126-GPOS-00066'
  tag fix_id: 'F-51941r779224_fix'
