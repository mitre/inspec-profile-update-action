control 'SV-42294' do
  title 'Classified Reproduction -  Written Procedures for SIPRNet Connected Classified Multi-Functional Devices (MFD) located in Space Not Approved for Collateral Classified Open Storage.
...
DoD 5200.22-M (NISPOM), Incorporating Change 2, 18 May 2016, Chapter 4, paragraph 4-102, and Chapter 5, Section 6 (Reproduction).'
  desc 'check', 'Classified Reproduction - Document Copying using Multi-Functional Device (MFD) machines (ie., printer, copier, fax, scanner) connected to SIPRNet.  This STIG Check concerns ONLY PROCEDURES for the reproduction of classified DOCUMENTS on Multi-Functional Devices (MFD) connected to the DoDIN.
...
NOTE:  Most MFD contain both hard drives (non-volatile memory) and volatile memory and cannot be properly sanitized of classified data or images to make the MFD unclassified.  Therefore, most (if not all) classified MFD should be housed and operated within space approved for collateral classified open storage.  If not maintained in spaces approved for classified open storage all MFD with non-volatile memory  that is used for classified reproduction must be under the continuous observation and control of a cleared person AT ALL TIMES.'
  ...
  tag check_id: 'C-40635r14_chk'
  tag severity: 'low'
  tag gid: 'V-31995'
  tag rid: 'SV-42294r3_rule'
  tag stig_id: 'IS-10.03.01'
  tag gtitle: 'Classified Reproduction - Written Procedures for SIPRNet Connected MFD'
  tag fix_id: 'F-35927r6_fix'
  tag 'documentable'
end
