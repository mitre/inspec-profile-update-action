...
OriginatingDoD Manual 5200.01, Volume 1, SUBJECT: DoD Information Security Program: Overview, Classification, and Declassification, Encl 2, para 9.j.(1) and Encl 3,  para 5.b., 7.b.(5), 12.e.
...
6.  If the grouping of U.S.-only workstations at a site is not operationally possible, the following steps must be taken:
    ...
    a. The U.S. command or agency must authorized an exception at the site, in writing, stating operational reasons for exception, and maintain the record of exception.  This exception must be approved by the appropriate CC/S/A level of command, which is normally a 3 or 4 star Flag Officer level.
...
7.  If a foreign national is permitted to view a US Only screen, U.S. personnel must first ensure:
  ...
  tag check_id: 'C-40629r36_chk'
  tag severity: 'high'
  tag gid: 'V-31991'
  tag rid: 'SV-42290r3_rule'
  tag stig_id: 'IS-08.01.01'
  tag gtitle: 'Classified Monitors/Displays (Physical Control of Classified Monitors)'
  tag fix_id: 'F-35923r6_fix'
  tag 'documentable'
  tag severity_override_guidance: 'The default finding for this requirement is a Category I severity level, but the following override guidance should be followed:  

There are ten individual checks and sub-checks associated with this requirement with five of these checks defaulting as CAT I.  

The other five checks, which are associated only with classified (SIPRNet) environments with a FN presence default as a CAT II finding.

If just one or all of the five CAT I default checks is found to be deficient then the finding will be a CAT I severity level.

If just one or all of the CAT II default checks is found to be deficient (*and none of the CAT I checks are found to be deficient) then the finding will be listed as a CAT II severity level.'
  tag potential_impacts: 'RELATED VULS (STIG ID):

1.  STIG ID: FN-04.01.01.  This requirement concerns two related concerns.  First is control of physical access to areas containing US Only workstations/monitor screens, equipment, media or documents in working environments where Foreign Nationals are employed or present. Second, It also covers maintaining continuous observation and control of US Only classified information system removable storage media and documents within classified storage locations (such as SCIFs, secure rooms or vaults) where foreign nationals are present OR placement in an approved safe.                         

2. STIG ID: IS-08.01.02.  This requirement concerns maintaining control of Common Access Cards (CACs), SIPRNet tokens AND locking of computer work stations/monitor screens when unattended by removal of CACs, SIPRNet tokens or using Ctrl/Alt/Del.

3. STIG ID: IS-08.03.01.  This requirement is specifically focused on checking written policy/procedures and initial/recurring training concerning cleared employee responsibilities and actions to protect classified work stations (monitor screens) under their control from unauthorized viewing.  This requirement includes environments where US Only monitors or Foreign Nationals are present.'
end
