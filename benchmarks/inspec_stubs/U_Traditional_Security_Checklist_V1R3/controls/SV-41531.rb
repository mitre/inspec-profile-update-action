...
4. Secondary doors (doors other than those secured with locks meeting FF-L-2740) shall be secured from the inside with deadbolt emergency egress hardware, a deadbolt, or a rigid wood or metal bar that extends across the width of the door. These deadbolt locks shall be secured when the combination lock on the primary door is spun.'
  ...
  tag check_id: 'C-39998r5_chk'
  tag severity: 'high'
  tag gid: 'V-31268'
  tag rid: 'SV-41531r3_rule'
  tag stig_id: 'IS-02.01.02'
  tag gtitle: 'Information Security (INFOSEC) - Secure Room Standards - Door Construction'
  tag fix_id: 'F-35165r5_fix'
  tag 'documentable'
  tag severity_override_guidance: 'Category I is the default severity level for this requirement.

There are four separate checks under this requirement and a deficiency found in all the checks  - except for check # 1 will result in a CAT I finding.  

If check #1 is the ONLY deficiency found pertaining to door construction then the severity level of the finding may be reduced to CAT II.  Following is the referenced CAT II requirement:

The doors to the room (primary and secondary) are not substantially constructed of wood or metal. (CAT II) 

If any or all of the other three checks under this requirement are found to be deficient the severity level will be CAT I.'
end
