...
NOTE:  Despite the mitigations in 2 and 3 above, there is still a concern that the procedure will not be accomplished promptly or successfully each time and that the risk for printed or copied classified documents to be left unattended for periods of time in the MFD machines still exists.  Therefore vulnerability still exists and must be considered as a potential finding.'
  ...
  tag check_id: 'C-40655r15_chk'
  tag severity: 'high'
  tag gid: 'V-32008'
  tag rid: 'SV-42324r3_rule'
  tag stig_id: 'IS-10.01.01'
  tag gtitle: 'Classified Reproduction - SIPRNet Connected Classified MFD in Space not Approved for Open Storage'
  tag fix_id: 'F-35957r6_fix'
  tag 'documentable'
  tag severity_override_guidance: 'Default finding is CAT I.

When a MFD (copier/printer/fax/scanner) connected to the SIPRNet is not properly located within a secret or higher collateral classified open storage area but is located within a secret or higher Controlled Access Area (CAA) the finding severity level may be reduced if the following procedures are accomplished:

May be mitigated to CAT II if it can be positively demonstrated there is no hard drive containing non-volatile memory in the device and that the volatile memory is purged/sanitized of all classified data or images after each use.  Powering down the machine will be a necessary part of this procedure to ensure that volatile memory is totally erased and sanitized so that it can be considered to be an unclassified device.  Documented procedures will also need to be available to support this process.

If a hard drive is present it may also be mitigated to a CAT II finding if the hard drive is promptly removed after each use and stored in an appropriate GSA approved container (safe).  Powering down the machine will also be a necessary part of this procedure to ensure that volatile memory is also erased and sanitized. Documented procedures will also need to be available to support this process.

NOTE:  Clearing of hard drives (such as by overwriting) is not adequate to sanitize a classified hard drive (magnetic or solid state) so that it can be deemed unclassified and left unattended in an area not approved for classified open storage.  This is regardless of the number of times the drive is over-written.  A hard drive (magnetic or solid state) can only be sanitized (made unclassified) by degaussing and/or physical destruction, thereby rending the drive no longer usable.  

ONLY volatile memory can be sanitized by removal of power without rendering the MFD unusable.'
end
