control 'SV-40729' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> “Include local directory path when uploading files to a server” must be “Enabled” and “Disable” selected from the drop-down box.
...
Criteria: If the value 160A is REG_DWORD = 3, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> “Include local directory path when uploading files to a server” to “Enabled” and select “Disable” from the drop-down box.'
  impact 0.5
  ref 'DPMS Target IE Version 9'
  tag check_id: 'C-39473r1_chk'
  tag severity: 'medium'
  tag gid: 'V-22153'
  tag rid: 'SV-40729r1_rule'
  tag stig_id: 'DTBI810'
  tag gtitle: 'DTBI810 - Local directory paths - Internet'
  tag fix_id: 'F-34591r1_fix'
  tag 'documentable'
