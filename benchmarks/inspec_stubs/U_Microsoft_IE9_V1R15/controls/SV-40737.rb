control 'SV-40737' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> “Include local directory path when uploading files to a server” must be “Enabled” and “Disable” selected from the drop-down box. 
...
Criteria: If the value 160A is REG_DWORD = 3, this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> “Include local directory path when uploading files to a server” to “Enabled” and select “Disable” from the drop-down box.'
  impact 0.5
  ref 'DPMS Target IE Version 9'
  tag check_id: 'C-39481r1_chk'
  tag severity: 'medium'
  tag gid: 'V-22158'
  tag rid: 'SV-40737r1_rule'
  tag stig_id: 'DTBI860'
  tag gtitle: 'DTBI860 - Local directory paths - Restricted'
  tag fix_id: 'F-34596r1_fix'
  tag 'documentable'
