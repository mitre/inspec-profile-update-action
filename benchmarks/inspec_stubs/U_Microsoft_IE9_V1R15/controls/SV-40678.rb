control 'SV-40678' do
  ...
  desc 'This policy setting allows you to manage whether .NET Framework components that are signed with Authenticode can be executed from Internet Explorer. These components include managed controls referenced from an object tag and managed executables referenced from a link. If you enable this policy setting, Internet Explorer will execute signed managed components. If you select Prompt in the drop-down box, Internet Explorer will prompt the user to determine whether to execute signed managed components. If you disable this policy setting, Internet Explorer will not execute signed managed components. If you do not configure this policy setting, Internet Explorer will execute signed managed components.'
...
Criteria: If the value 2001 is REG_DWORD = 3, this is not a finding.'
  ...
  tag check_id: 'C-39408r1_chk'
  tag severity: 'medium'
  tag gid: 'V-15561'
  tag rid: 'SV-40678r1_rule'
  tag stig_id: 'DTBI655'
  tag gtitle: 'DTBI655 - .NET w/Authenticode signed - Restricted'
  tag fix_id: 'F-34532r1_fix'
  tag 'documentable'
