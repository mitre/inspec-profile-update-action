control 'SV-40681' do
  ...
  desc 'check', 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Settings -> Component Updates -> Periodic check for updates to Internet Explorer and Internet Tools -> "Turn off configuring the update check interval (in days)" must be “Enabled” and "30" selected from the drop-down box.

Procedure: Use the Windows Registry Editor to navigate to the following key:
HKLM\\Software\\Policies\\Microsoft\\Internet Explorer\\Main 

Criteria: If the value Update_Check_Interval is REG_DWORD = 30 (Decimal), this is not a finding.'
  desc 'fix', 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Settings -> Component Updates -> Periodic check for updates to Internet Explorer and Internet Tools -> "Turn off configuring the update check interval (in days)" to “Enabled” and select "30" from the drop-down box.'
  impact 0.5
  ref 'DPMS Target IE Version 9'
  tag check_id: 'C-39411r2_chk'
  tag severity: 'medium'
  tag gid: 'V-15564'
  tag rid: 'SV-40681r1_rule'
  tag stig_id: 'DTBI680'
  tag gtitle: 'DTBI680 - Update check interval'
  tag fix_id: 'F-34535r1_fix'
  tag 'documentable'
