control 'SRG-NET-000522-VVSM-00010_rule' do
  title 'When using locally stored user accounts, the Unified Communications Session Manager must store only cryptographic representations of passwords.'
  desc %q(If passwords and PSKs are not encrypted when stored, they may be read if the storage location is compromised. 
...
This requirement applies to all accounts including authentication server; Authentication, Authorization, and Accounting (AAA), and local accounts, including the root account, and the account of last resort.)
  desc 'check', 'Verify the Unified Communications Session Manager, when using locally stored user accounts, is configured to only store cryptographic representations of passwords.

If the Unified Communications Session Manager is not configured to only store cryptographic representations of passwords, this is a finding.'
  desc 'fix', 'Configure the Unified Communications Session Manager, when using locally stored user accounts, to only store cryptographic representations of passwords.'
  impact 0.5
  tag check_id: 'C-SRG-NET-000522-VVSM-00010_chk'
  tag severity: 'medium'
  tag gid: 'SRG-NET-000522-VVSM-00010'
  tag rid: 'SRG-NET-000522-VVSM-00010_rule'
  tag stig_id: 'SRG-NET-000522-VVSM-00010'
  tag gtitle: 'SRG-NET-000522-VVSM-00010'
  tag fix_id: 'F-SRG-NET-000522-VVSM-00010_fix'
  tag 'documentable'
  tag cci: ['CCI-000196']
  tag nist: ['IA-5 (1) (c)']
end
