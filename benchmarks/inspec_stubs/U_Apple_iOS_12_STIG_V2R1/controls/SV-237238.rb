control 'SV-237238' do
  title 'Apple iOS must be configured to not allow passwords that include more than two repeating or sequential characters.'
...
If "Allow simple value" is checked in the Apple iOS management tool, "<key>allowSimple</key> <true/>" appears in the Configuration Profile, or the password policy on the Apple iOS device from the Apple iOS management tool does not have "Simple passcodes allowed" set to "No", this is a finding.'
  ...
  tag check_id: 'C-40457r642264_chk'
  tag severity: 'low'
  tag gid: 'V-237238'
  tag rid: 'SV-237238r642266_rule'
  tag stig_id: 'AIOS-12-000200'
  tag gtitle: 'PP-MDF-301020'
  tag fix_id: 'F-40420r642265_fix'
  tag 'documentable'
  tag legacy: ['SV-96471', 'V-81757']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
