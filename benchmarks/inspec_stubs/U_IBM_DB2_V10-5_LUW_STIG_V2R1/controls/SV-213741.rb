control 'SV-213741' do
  title 'DB2 must generate audit records when privileges/permissions are modified.'
...
In an SQL environment, modifying permissions is typically done via the GRANT and REVOKE."
  desc 'check', "To verify the database generates audit records when the database privileges/permissions are modified the SECMAINT, CONTEXT category auditing must be implemented at the database level. 
...
If the values for CONTEXTSTATUS and SECMAINTSTATUS are not 'B' (Both) or ERRORTYPE is not 'A' (Audit), this is a finding."
  desc 'fix', 'If a correct audit policy does not exist, define an audit policy with the needed subset using the CREATE AUDIT POLICY SQL statement: 
...
DB2> AUDIT DATABASE USING POLICY <DB audit policy name>'
  ...
  tag check_id: 'C-14962r295272_chk'
  tag severity: 'medium'
  tag gid: 'V-213741'
  tag rid: 'SV-213741r879866_rule'
  tag stig_id: 'DB2X-00-010200'
  tag gtitle: 'SRG-APP-000495-DB-000328'
  tag fix_id: 'F-14960r295273_fix'
  tag 'documentable'
  tag legacy: ['SV-89299', 'V-74625']
  tag cci: ['CCI-000172']
  tag nist: ['AU-12 c']
end
