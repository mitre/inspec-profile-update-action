control 'WDNS-22-000013_rule' do
  title 'The Windows 2022 DNS Server must implement cryptographic mechanisms to detect changes to information during transmission.'
...
Confidentiality is not an objective of DNS, but integrity is. DNSSEC and TSIG/SIG(0) both digitally sign DNS information to authenticate its source and ensure its integrity.'
  desc 'check', 'Note: If the Windows DNS Server hosts only Active Directory (AD)-integrated zones and does not host any file-based zones, this is not applicable. 
...
From the expanded list, right-click to select the zone (repeat for each hosted zone), point to DNSSEC, and then click "Sign the Zone" using either approved saved parameters or approved custom parameters.'
  impact 0.5
  tag check_id: 'C-WDNS-22-000013_chk'
  tag severity: 'medium'
  tag gid: 'WDNS-22-000013'
  tag rid: 'WDNS-22-000013_rule'
  tag stig_id: 'WDNS-22-000013'
  tag gtitle: 'SRG-APP-000440-DNS-000065'
  tag fix_id: 'F-WDNS-22-000013_fix'
  tag 'documentable'
  tag cci: ['CCI-002421']
  tag nist: ['SC-8 (1)']
end
