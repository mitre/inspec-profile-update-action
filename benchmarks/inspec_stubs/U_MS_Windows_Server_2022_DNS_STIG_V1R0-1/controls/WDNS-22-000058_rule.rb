control 'WDNS-22-000058_rule' do
  title 'The Windows DNS secondary server must request data integrity verification from the primary server when requesting name/address resolution.'
...
From the expanded list, right-click to select the zone (repeat for each hosted zone), point to DNSSEC, and then click "Sign the Zone" using either approved saved parameters or approved custom parameters.'
  impact 0.5
  tag check_id: 'C-WDNS-22-000058_chk'
  tag severity: 'medium'
  tag gid: 'WDNS-22-000058'
  tag rid: 'WDNS-22-000058_rule'
  tag stig_id: 'WDNS-22-000058'
  tag gtitle: 'SRG-APP-000424-DNS-000057'
  tag fix_id: 'F-WDNS-22-000058_fix'
  tag 'documentable'
  tag cci: ['CCI-002466']
  tag nist: ['SC-21']
end
