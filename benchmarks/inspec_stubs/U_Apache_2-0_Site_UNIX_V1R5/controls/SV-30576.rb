control 'SV-30576' do
  ...
  desc 'A symbolic link allows a file or a directory to be referenced using a symbolic name raising a potential hazard if symbolic linkage is made to a sensitive area.
...
When web scripts are executed and symbolic links are allowed, the web user could be allowed to access locations on the web server that are outside the scope of the web document root or home directory.'
  desc 'check', 'Locate the directories containing the web content, (i.e., /usr/local/apache/htdocs). 
...
Finally, the target file or directory must be owned by the same owner as the link, which should be a privileged account with access to the web content.'
  desc 'fix', 'Disable symbolic links.'
  impact 0.7
  ref 'DPMS Target Apache Site 2.0'
  tag check_id: 'C-31108r1_chk'
  tag severity: 'high'
  tag gid: 'V-2227'
  ...
  tag 'documentable'
  tag severity_override_guidance: 'If symbolic links are found in the web content directory tree, the target file or directory is outside of the web content directory tree, and file permissions allow the web user write authority, then the severity level will remain at CAT 1. 
