control 'SV-234923' do
  title 'The SUSE operating system must generate audit records for all uses of the lsetxattr system call.'
  desc 'Without generating audit records specific to the security and mission needs of the organization, it would be difficult to establish, correlate, and investigate the events relating to an incident or identify those responsible for one.

Audit records can be generated from various components within the information system (e.g., module or policy filter).

'
  desc 'check', %q(Verify the SUSE operating system generates an audit record for all uses of the "lsetxattr" system call.

Check that the system call is being audited by performing the following command:

> sudo auditctl -l | grep -w 'lsetxattr'
...
The "-k" allows for specifying an arbitrary identifier. The string following "-k" does not need to match the example output above.)
  desc 'fix', 'Configure the SUSE operating system to generate an audit record for all uses of the "lsetxattr" system call. 
...
> sudo augenrules --load'
  ...
  tag check_id: 'C-38111r619038_chk'
  tag severity: 'medium'
  tag gid: 'V-234923'
  tag rid: 'SV-234923r622137_rule'
  tag stig_id: 'SLES-15-030240'
  tag gtitle: 'SRG-OS-000037-GPOS-00015'
  tag fix_id: 'F-38074r619039_fix'
