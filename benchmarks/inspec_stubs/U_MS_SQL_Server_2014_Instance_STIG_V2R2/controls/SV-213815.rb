...
If SQL Server Audit is in use, check to see that all audit records include enough information to establish the sources of the events; if not, this is a finding."
  desc 'fix', 'Design and deploy a SQL Server Audit or Trace that captures the NT User Name, NT Domain Name, Host Name, Client Process ID, Application Name, Login Name, SPID, DB User Name, and Login SID (each where relevant) for all auditable events.  
...
If SQL Server Audit is intended to be in use, design and deploy an Audit that captures all auditable events. The code provided in the supplemental file Audit.sql can be used as the basis for creating an Audit.'
  ...
  tag check_id: 'C-15034r312796_chk'
  tag severity: 'medium'
  tag gid: 'V-213815'
  tag rid: 'SV-213815r395730_rule'
  tag stig_id: 'SQL4-00-012100'
  tag gtitle: 'SRG-APP-000098-DB-000042'
  tag fix_id: 'F-15032r312797_fix'
  tag 'documentable'
  tag legacy: ['SV-82267', 'V-67777']
  tag cci: ['CCI-000133']
  tag nist: ['AU-3 d']
end
