control 'SV-237615' do
  title 'The SUSE operating system must have system commands owned by root.'
...
This requirement applies to SUSE operating systems with software libraries that are accessible and configurable, as in the case of interpreted languages. Software libraries also include privileged programs which execute with escalated privileges. Only qualified and authorized individuals must be allowed to obtain access to information system components for purposes of initiating changes, including upgrades and modifications.'
  desc 'check', %q(Verify the system commands contained in the following directories are owned by root:
...
If any system commands are returned, this is a finding.)
  desc 'fix', "Configure the system commands - and their respective parent directories - to be protected from unauthorized access. Run the following command:

> sudo find -L /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin ! -user root -type f -exec chown root '{}' \\;"
  impact 0.5
  ref 'DPMS Target SUSE Linux Enterprise Server 12'
  tag check_id: 'C-40834r646806_chk'
  tag severity: 'medium'
  tag gid: 'V-237615'
  tag rid: 'SV-237615r646808_rule'
  tag stig_id: 'SLES-12-010879'
  tag gtitle: 'SRG-OS-000259-GPOS-00100'
  tag fix_id: 'F-40797r646807_fix'
  tag 'documentable'
  tag cci: ['CCI-001499']
  tag nist: ['CM-5 (6)']
end
