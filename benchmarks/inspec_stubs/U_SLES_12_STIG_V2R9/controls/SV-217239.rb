control 'SV-217239' do
  title 'The SUSE operating system must generate audit records for a uses of the chsh command.'
  desc 'Reconstruction of harmful events or forensic analysis is not possible if audit records do not contain enough information.

At a minimum, the organization must audit the full-text recording of privileged commands. The organization must maintain audit trails in sufficient detail to reconstruct events to determine the cause and impact of compromise.

'
  desc 'check', 'Verify the SUSE operating system generates an audit record for all uses of the "chsh" command.
...
If the command does not return any output or the returned line is commented out, this is a finding.'
  desc 'fix', 'Configure the SUSE operating system to generate an audit record for all uses the "chsh" command.
...
# sudo systemctl restart auditd.service'
  ...
  tag check_id: 'C-18467r622382_chk'
  tag severity: 'low'
  tag gid: 'V-217239'
  tag rid: 'SV-217239r854133_rule'
  tag stig_id: 'SLES-12-020580'
  tag gtitle: 'SRG-OS-000037-GPOS-00015'
  tag fix_id: 'F-18465r622383_fix'
