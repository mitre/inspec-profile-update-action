control 'SV-255170' do
  title 'Microsoft Android 11 must be configured to lock the display after 15 minutes (or less) of inactivity.'
...
SFR ID: FMT_SMF_EXT.1.1 #2b'
  desc 'check', 'Review Microsoft Android device configuration settings to determine if the mobile device has the screen lock timeout set to 15 minutes or less.
...
3. Verify that "Max time to screen lock" is set to any number between 1 and 900. Units are in seconds, so 900 represents 15 minutes.

On the Microsoft Android 11 device:
1. Open Settings >> Display.
2. Tap "Screen timeout".
3. Ensure the Screen timeout value is set to a period of 15 minutes or less.
...
If the EMM console device policy is not set to 15 minutes or less for the screen lock timeout or on the Android 11 device, the device policy is not set to 15 minutes or less for the screen lock timeout, this is a finding.'
  desc 'fix', 'Configure the Microsoft Android 11 device to lock the device display after 15 minutes (or less) of inactivity.
...
3. Set "Max time to screen lock" to any number between 1 and 900. Units are in seconds, so 900 represents 15 minutes (15 * 60 seconds).'
  ...
  tag check_id: 'C-58783r870648_chk'
  tag severity: 'medium'
  tag gid: 'V-255170'
  tag rid: 'SV-255170r870650_rule'
  tag stig_id: 'MSFT-11-000400'
  tag gtitle: 'PP-MDF-301040'
  tag fix_id: 'F-58727r870649_fix'
  tag 'documentable'
  tag cci: ['CCI-000057']
  tag nist: ['AC-11 a']
end
