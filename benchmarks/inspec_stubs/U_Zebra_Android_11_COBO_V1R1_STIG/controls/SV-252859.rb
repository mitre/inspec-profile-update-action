control 'SV-252859' do
  title 'Zebra Android 11 must be configured to not display the following (work profile) notifications when the device is locked: [selection:
...
SFR ID: FMT_SMF_EXT.1.1 #19'
  desc 'check', 'Review Zebra Android device settings to determine if the Zebra Android device displays (work container) notifications on the lock screen. Notifications of incoming phone calls are acceptable even when the device is locked. 
...
If the EMM console device policy allows work notifications on the lock screen, or the Android 11 device allows work notifications on the lock screen, this is a finding.'
  desc 'fix', 'Configure the Zebra Android 11 device to not display (work profile) notifications when the device is locked.
...
3. Toggle "Disable Unredacted Notifications".'
  ...
  tag check_id: 'C-56315r820502_chk'
  tag severity: 'medium'
  tag gid: 'V-252859'
  tag rid: 'SV-252859r820504_rule'
  tag stig_id: 'ZEBR-11-001600'
  tag gtitle: 'PP-MDF-301120'
  tag fix_id: 'F-56265r820503_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
