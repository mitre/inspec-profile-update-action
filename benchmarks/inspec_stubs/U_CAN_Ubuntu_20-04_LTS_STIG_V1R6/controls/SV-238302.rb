...
'
  desc 'check', 'Verify the Ubuntu operating system configures the audit tools to be group-owned by root to prevent any unauthorized access. 
...
If any of the audit tools are not group-owned by root, this is a finding.'
  desc 'fix', 'Configure the audit tools on the Ubuntu operating system to be protected from unauthorized access by setting the file group as  root using the following command: 
...
Replace "[audit_tool]" with each audit tool not group-owned by root.'
  ...
  tag check_id: 'C-41512r654079_chk'
  tag severity: 'medium'
  tag gid: 'V-238302'
