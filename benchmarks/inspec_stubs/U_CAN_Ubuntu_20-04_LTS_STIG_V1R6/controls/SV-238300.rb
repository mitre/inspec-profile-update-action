...
'
  desc 'check', 'Verify the Ubuntu operating system configures the audit tools to have a file permission of 0755 or less to prevent unauthorized access by running the following command: 
...
If any of the audit tools have a mode more permissive than 0755, this is a finding.'
  desc 'fix', 'Configure the audit tools on the Ubuntu operating system to be protected from unauthorized access by setting the correct permissive mode using the following command: 
...
Replace "[audit_tool]" with the audit tool that does not have the correct permissions.'
  ...
  tag check_id: 'C-41510r654073_chk'
  tag severity: 'medium'
  tag gid: 'V-238300'
