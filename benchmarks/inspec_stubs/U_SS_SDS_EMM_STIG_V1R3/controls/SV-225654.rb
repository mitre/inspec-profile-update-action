control 'SV-225654' do
  title 'The Samsung SDS EMM must limit the number of concurrent sessions to one session for all accounts and/or account types.'
...
'
  desc 'check', 'Review the Samsung SDS EMM configuration settings and verify the server is configured to limit the number of concurrent sessions to one session for all accounts and/or account types.
...
If the MDM console Multiple login is not set to "Disallow", this is a finding.'
  desc 'fix', 'Configure the Samsung SDS EMM to limit the number of concurrent sessions to one session all accounts and/or account types.
...
2. Go to Setting >> Server >> Configuration and check Multiple login as "Disallow."'
  ...
  tag check_id: 'C-27355r835011_chk'
  tag severity: 'medium'
  tag gid: 'V-225654'
  tag rid: 'SV-225654r835013_rule'
  tag stig_id: 'SSDS-00-200070'
  tag gtitle: 'PP-MDM-431010'
  tag fix_id: 'F-27343r835012_fix'
