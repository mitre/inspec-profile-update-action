...
The fix for this vulnerability specifies the use of REVOKE.  Be aware that revoking a permission that is currently denied to a role or user does not necessarily disable the permission.  If the user or role can inherent the permission from another role, revoking the denied permission from the user or the first role can effectively enable the inherited permission."
  desc 'check', "Obtain the list of roles that are authorized for the SQL Server 'Alter any server audit' permission and what 'Grant', 'Grant With', and/or 'Deny' privilege is authorized. Obtain the list of roles with that permission by running the following query:
...
If any role has 'Grant', 'With Grant' or 'Deny' privileges on this permission and users with that role are not authorized to have the permission, this is a finding."
  desc 'fix', "Remove the 'Alter any server audit' permission access from the role that is not authorized  by executing the following query:


REVOKE Alter any server audit TO <'role name'>"
  impact 0.5
  ref 'DPMS Target SQL Server Installation 2012'
  tag check_id: 'C-47828r3_chk'
  tag severity: 'medium'
  tag gid: 'V-41260'
  tag rid: 'SV-53742r3_rule'
  tag stig_id: 'SQL2-00-006600'
  tag gtitle: 'SRG-APP-000035-DB-000007'
  tag fix_id: 'F-46651r1_fix'
  tag cci: ['CCI-003014']
  tag nist: ['AC-3 (3)']
end
