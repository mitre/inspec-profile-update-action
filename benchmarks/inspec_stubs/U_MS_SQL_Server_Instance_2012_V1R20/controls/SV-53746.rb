...
Note that this does not apply to logins with names of the form '##MS...##'.  These accounts are internal-use system principals provisioned by the DBMS, and required by it for specific purposes."
  desc 'check', "Obtain the list of accounts that have direct access to the server-level permission 'Create endpoint' by running the following query:
...
;
GO"
  desc 'fix', "Remove the 'Create endpoint' permission access from the account that has direct access by running the following script:
...
GO"
  ...
  tag check_id: 'C-47832r5_chk'
  tag severity: 'medium'
  tag gid: 'V-41264'
  tag rid: 'SV-53746r4_rule'
  tag stig_id: 'SQL2-00-006200'
  tag gtitle: 'SRG-APP-000035-DB-000007'
  tag fix_id: 'F-46655r1_fix'
  tag cci: ['CCI-003014']
  tag nist: ['AC-3 (3)']
end
