...
The fix for this vulnerability specifies the use of REVOKE.  Be aware that revoking a permission that is currently denied to a role or user does not necessarily disable the permission.  If the user or role can inherent the permission from another role, revoking the denied permission from the user or the first role can effectively enable the inherited permission."
  desc 'check', "Obtain the list of roles that are authorized for the SQL Server 'Administer bulk operations' permission and what 'Grant', 'Grant With', and/or 'Deny' privilege is authorized.
...
;
GO"
  desc 'fix', "Remove the 'Administer bulk operations' permission access from the role that is not authorized by executing the following query:


REVOKE Administer bulk operations TO <'role name'>"
  impact 0.5
  ref 'DPMS Target SQL Server Installation 2012'
  tag check_id: 'C-47837r5_chk'
  tag severity: 'medium'
  tag gid: 'V-41269'
  tag rid: 'SV-53751r4_rule'
  tag stig_id: 'SQL2-00-005700'
  tag gtitle: 'SRG-APP-000035-DB-000007'
  tag fix_id: 'F-46660r1_fix'
  tag cci: ['CCI-003014']
  tag nist: ['AC-3 (3)']
end
