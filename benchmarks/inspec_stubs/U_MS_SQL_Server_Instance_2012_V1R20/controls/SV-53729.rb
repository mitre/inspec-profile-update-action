...
Note that this does not apply to logins with names of the form '##MS...##'.  These accounts are internal-use system principals provisioned by the DBMS, and required by it for specific purposes."
  desc 'check', "Obtain the list of accounts that have direct access to the server-level permission 'Alter server state' by running the following query:
...
;
GO"
  desc 'fix', "Remove the 'Alter server state' permission access from the account that has direct access by running the following script:
...
GO"
  ...
  tag check_id: 'C-47815r5_chk'
  tag severity: 'medium'
  tag gid: 'V-41248'
  tag rid: 'SV-53729r4_rule'
  tag stig_id: 'SQL2-00-007800'
  tag gtitle: 'SRG-APP-000035-DB-000007'
  tag fix_id: 'F-46638r1_fix'
  tag cci: ['CCI-003014']
  tag nist: ['AC-3 (3)']
end
