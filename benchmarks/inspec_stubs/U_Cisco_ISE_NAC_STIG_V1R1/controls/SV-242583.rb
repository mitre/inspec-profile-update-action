control 'SV-242583' do
  title 'The Cisco ISE must be configured so that all endpoints that are allowed to bypass policy assessment are approved by the Information System Security Manager (ISSM) and documented in the System Security Plan (SSP).'
  ...
  desc 'check', 'Review the posture policy to ensure mandated endpoints are being assed and if there are exceptions to the policy that they are documented and approved by the ISSM. 
...
If the policy will not be applied to required endpoints or if exempted endpoints are not approved and documented, this is a finding.'
  desc 'fix', 'Configure the posture policy to assess mandated endpoints. 
...
Note: For exceptions, a condition can be made to "Not Equal" or "Not Contains" a pattern to exempt devices from the policy.'
  ...
  tag check_id: 'C-45858r714057_chk'
  tag severity: 'medium'
  tag gid: 'V-242583'
  tag rid: 'SV-242583r714059_rule'
  tag stig_id: 'CSCO-NC-000090'
  tag gtitle: 'SRG-NET-000015-NAC-000080'
  tag fix_id: 'F-45815r714058_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
