...
To support this requirement, the operating system may have an integrated solution incorporating continuous scanning using HBSS and periodic scanning using other tools, as specified in the requirement.'
  desc 'check', 'The Ubuntu operating system is not compliant with this requirement; hence, it is a finding. However, the severity level can be mitigated to a CAT III if the ENSLTP module is installed and running. 
...
If the daemon is not running, this finding will remain as a CAT II.'
  desc 'fix', 'The Ubuntu operating system is not compliant with this requirement; however, the severity level can be mitigated to a CAT III if the ENSLTP module is installed and running. 
...
# sudo apt-get install mfetp'
  ...
  tag check_id: 'C-41546r654181_chk'
  tag severity: 'low'
  tag gid: 'V-238336'
