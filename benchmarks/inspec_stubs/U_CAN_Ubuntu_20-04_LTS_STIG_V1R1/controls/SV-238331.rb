control 'SV-238331' do
  ...
  desc "Emergency accounts are different from infrequently used accounts (i.e., local logon accounts used by the organization's System Administrator
...
If any of these accounts do not expire within 72 hours of that account's creation, this is a finding."
  desc 'fix', 'If an emergency account must be created, configure the system to terminate the account after a 72-hour time period with the following command to set an expiration date on it. Substitute "account_name" with the account to be created. 
 
$ sudo chage -E $(date -d "+3 days" +%F) account_name'
  impact 0.3
  ref 'DPMS Target Canonical Ubuntu 20.04 LTS'
  tag check_id: 'C-41541r654166_chk'
  tag severity: 'low'
  tag gid: 'V-238331'
