control 'SV-238233' do
  ...
  desc 'check', %q(Verify the Ubuntu operating system, for PKI-based authentication, uses local revocation data when unable to access it from the network.  
...
If the system is missing an "/etc/pam_pkcs11/" directory and an "/etc/pam_pkcs11/pam_pkcs11.conf", find an example to copy into place and modify accordingly at "/usr/share/doc/libpam-pkcs11/examples/pam_pkcs11.conf.example.gz".'
  ...
  tag check_id: 'C-41443r653872_chk'
  tag severity: 'medium'
  tag gid: 'V-238233'
