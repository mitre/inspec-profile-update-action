control 'SV-224768' do
  title 'When using PKI-based authentication for user access, the ISEC7 EMM Suite must validate certificates by constructing a certification path (which includes status information) to an accepted trust anchor.'
...
This requirement verifies that a certification path to an accepted trust anchor is used for certificate validation and that the path includes status information. Path validation is necessary for a relying party to make an informed trust decision when presented with any certificate not already explicitly trusted. Status information for certification paths includes certificate revocation lists or online certificate status protocol responses. Validation of the certificate status information is out of scope for this requirement.'
  desc 'check', 'Login to the server(s) hosting the ISEC7 EMM Suite application.
...
If the DoD Root PKI Certificates Authorities have not been added to the server, this is a finding.'
  desc 'fix', 'Login to the server(s) hosting the ISEC7 EMM Suite application.
Open the Microsoft Management Console and add the Local Computer Certificates snap-in.
Open the Trusted Root Certification Authorities >> Certificates.
Install the DoD Root PKI Certificates Authorities to the server.'
  impact 0.5
  ref 'DPMS Target ISEC7 Sphere'
  tag check_id: 'C-26459r461560_chk'
  tag severity: 'medium'
  tag gid: 'V-224768'
  tag rid: 'SV-224768r505933_rule'
  tag stig_id: 'ISEC-06-000780'
  tag gtitle: 'SRG-APP-000175'
  tag fix_id: 'F-26447r461561_fix'
  tag 'documentable'
  tag legacy: ['SV-106499', 'V-97395']
  tag cci: ['CCI-000185']
  tag nist: ['IA-5 (2) (b) (1)']
end
