control 'SV-87329' do
  title 'The Cassandra Server must generate audit records when privileges/permissions are added.'
...
In an SQL environment, adding permissions is typically done via the GRANT command, or, in the negative, the DENY command."
  desc 'check', %q(Review the Cassandra Server configuration to ensure audit records are generated when privileges/permissions are added.
...
If level is not set to "ALL", this is a finding.)
  desc 'fix', %q(Configure the Cassandra Server to generate audit records when privileges/permissions are added.
...
# sed -i 's/^\(\s*\\)<root level=".*">\(\s*\\)$/\1<root level="ALL">\2/' /usr/lib/vmware-vcops/user/conf/cassandra/logback.xml)
  ...
  tag check_id: 'C-72853r1_chk'
  tag severity: 'medium'
  tag gid: 'V-72697'
  tag rid: 'SV-87329r1_rule'
  tag stig_id: 'VROM-CS-000285'
  tag gtitle: 'SRG-APP-000495-DB-000326'
  tag fix_id: 'F-79101r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000172']
  tag nist: ['AU-12 c']
end
