control 'SV-87333' do
  title 'The Cassandra Server must generate audit records when privileges/permissions are modified.'
...
In an SQL environment, modifying permissions is typically done via the GRANT, REVOKE, and DENY commands."
  desc 'check', %q(Review the Cassandra Server configuration to ensure audit records are generated when privileges/permissions are modified.
...
If level is not set to "ALL", this is a finding.)
  desc 'fix', %q(Configure the Cassandra Server to generate audit records when privileges/permissions are modified.
...
# sed -i 's/^\(\s*\\)<root level=".*">\(\s*\\)$/\1<root level="ALL">\2/' /usr/lib/vmware-vcops/user/conf/cassandra/logback.xml)
  ...
  tag check_id: 'C-72857r1_chk'
  tag severity: 'medium'
  tag gid: 'V-72701'
  tag rid: 'SV-87333r1_rule'
  tag stig_id: 'VROM-CS-000295'
  tag gtitle: 'SRG-APP-000495-DB-000328'
  tag fix_id: 'F-79105r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000172']
  tag nist: ['AU-12 c']
end
