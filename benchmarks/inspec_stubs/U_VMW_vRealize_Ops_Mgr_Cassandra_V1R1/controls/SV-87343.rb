control 'SV-87343' do
  title 'The Cassandra Server must generate audit records when unsuccessful attempts to delete privileges/permissions occur.'
...
To aid in diagnosis, it is necessary to keep track of failed attempts in addition to the successful ones."
  desc 'check', %q(Review the Cassandra Server configuration to ensure audit records are generated when unsuccessful attempts to delete privileges/permissions occur.
...
If level is not set to "ALL", this is a finding.)
  desc 'fix', %q(Configure the Cassandra Server to generate audit records when unsuccessful attempts to delete privileges/permissions occur.
...
# sed -i 's/^\(\s*\\)<root level=".*">\(\s*\\)$/\1<root level="ALL">\2/' /usr/lib/vmware-vcops/user/conf/cassandra/logback.xml)
  ...
  tag check_id: 'C-72867r1_chk'
  tag severity: 'medium'
  tag gid: 'V-72711'
  tag rid: 'SV-87343r1_rule'
  tag stig_id: 'VROM-CS-000330'
  tag gtitle: 'SRG-APP-000499-DB-000331'
  tag fix_id: 'F-79115r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000172']
  tag nist: ['AU-12 c']
end
