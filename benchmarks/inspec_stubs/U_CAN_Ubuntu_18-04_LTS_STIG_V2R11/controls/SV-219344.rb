...
This requirement applies to the Ubuntu operating system performing security function verification/testing and/or systems and environments that require this functionality.'
  desc 'check', 'Verify that Advanced Intrusion Detection Environment (AIDE) performs a verification of the operation of security functions every 30 days.
...
If the "/etc/cron.daily/aide" file does not exist or a cron job is not configured to run at least every 30 days, this is a finding.'
  desc 'fix', 'The cron file for AIDE is fairly complex as it creates the report. This file is installed with the aide-common package and the default can be restored by copying it from another location:

# sudo cp /usr/share/aide/config/cron.daily/aide /etc/cron.daily/aide'
  impact 0.5
  ref 'DPMS Target Canonical Ubuntu 18.04 LTS'
  tag check_id: 'C-21069r305360_chk'
  tag severity: 'medium'
  tag gid: 'V-219344'
  tag rid: 'SV-219344r853403_rule'
  tag stig_id: 'UBTU-18-010516'
  tag gtitle: 'SRG-OS-000446-GPOS-00200'
  tag fix_id: 'F-21068r305361_fix'
  tag 'documentable'
  tag legacy: ['SV-110013', 'V-100909']
  tag cci: ['CCI-002699']
  tag nist: ['SI-6 b']
end
