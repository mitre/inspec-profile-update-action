...
Alerts may be transmitted, for example, telephonically, by electronic mail messages, or by text messaging. The ALG must either send the alert to a management console that is actively monitored by authorized personnel or use a messaging capability to send the alert directly to designated personnel.)
  desc 'check', 'In the DataPower WebGUI, navigate to Administration >> Access >> SNMP Settings. On the "Trap Event Subscriptions" tab, verify the inclusion of Event Subscriptions that are judged to be associated with the detection of root level intrusion events that provide unauthorized privileged access.
...
If no SNMP Server is configured as a Trap and Notification Target, this is a finding.'
  desc 'fix', 'In the DataPower WebGUI, navigate to Administration >> Access >> SNMP Settings. Configure the "Trap Event Subscriptions" tab to include Event Subscriptions that are judged to be associated with the detection of user level intrusions which provide non-privileged access. Configure the "Trap and Notification Targets" tab to include an approved SNMP server that generates alerts that will be forwarded, at a minimum, to the ISSO and ISSM.'
  impact 0.5
  ref 'DPMS Target IBM DataPower XI52 ALG'
  tag check_id: 'C-65927r2_chk'
  tag severity: 'medium'
  tag gid: 'V-65299'
  tag rid: 'SV-79789r1_rule'
  tag stig_id: 'WSDP-AG-000116'
  tag gtitle: 'SRG-NET-000392-ALG-000147'
  tag fix_id: 'F-71239r1_fix'
  tag 'documentable'
  tag cci: ['CCI-002664']
  tag nist: ['SI-4 (5)']
end
