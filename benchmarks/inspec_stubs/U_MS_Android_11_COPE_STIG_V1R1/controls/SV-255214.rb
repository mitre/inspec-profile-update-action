...
SFR ID: FMT_SMF_EXT.1.1 #36)
  desc 'check', 'The DOD warning banner can be displayed by either of the following methods (required text is found in the Vulnerability Description):

1. By placing the DOD warning banner text in the user agreement signed by each Microsoft Android device user (preferred method).
2. By configuring the warning banner text on the EMM console and installing the banner on each managed mobile device.

Determine which method is used at the Microsoft Android device site and follow the appropriate validation procedure below.

Validation Procedure for Method #1:
Review the signed user agreements for several Microsoft Android device users and verify the agreement includes the required DOD warning banner text.
...
1. By placing the DOD warning banner text in the user agreement signed by each Microsoft Android 11 device user (preferred method).
2. By configuring the warning banner text on the EMM console and installing the banner on each managed mobile device.
...
4. Enter in message.'
  ...
  tag check_id: 'C-58827r870744_chk'
  tag severity: 'low'
  tag gid: 'V-255214'
  tag rid: 'SV-255214r870827_rule'
  tag stig_id: 'MSFT-11-003400'
  tag gtitle: 'PP-MDF-301200'
  tag fix_id: 'F-58771r870745_fix'
  tag 'documentable'
  tag cci: ['CCI-000048']
  tag nist: ['AC-8 a']
end
