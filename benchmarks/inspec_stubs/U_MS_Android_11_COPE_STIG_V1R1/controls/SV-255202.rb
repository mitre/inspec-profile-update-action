control 'SV-255202' do
  title 'Microsoft Android 11 must be configured to not allow passwords that include more than two repeating or sequential characters.'
...
SFR ID: FMT_SMF_EXT.1.1 #1b'
  desc 'check', 'Review Microsoft Android device configuration settings to determine if the mobile device is prohibiting passwords with more than two repeating or sequential characters.
...
Note: Alphabetic, Alphanumeric, and Complex are also acceptable selections but these selections will cause the user to select a complex password, which is not required by the STIG.'
  desc 'fix', 'Configure the Microsoft Android 11 device to prevent passwords from containing more than two repeating or sequential characters.
...
Note: Alphabetic, Alphanumeric, and Complex are also acceptable selections but these selections will cause the user to select a complex password, which is not required by the STIG.'
  ...
  tag check_id: 'C-58815r870711_chk'
  tag severity: 'medium'
  tag gid: 'V-255202'
  tag rid: 'SV-255202r870815_rule'
  tag stig_id: 'MSFT-11-000200'
  tag gtitle: 'PP-MDF-301020'
  tag fix_id: 'F-58759r869222_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
