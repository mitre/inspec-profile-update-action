control 'SV-95067' do
  title 'The Samsung Android 8 with Knox CONTAINER must implement the management setting: Disable Allow New Admin Install.'
...
SFR ID: FMT_SMF_EXT.1.1 #47"
  desc 'check', 'Review Samsung Android 8 with Knox CONTAINER configuration settings to determine if the mobile device is configured to disallow new admin installations. 
...
If the MDM console "Prevent New Admin Install" check box is not selected or on the Samsung Android 8 with Knox device CONTAINER, the user is able to install another application requiring admin permissions on the device, this is a finding.'
  desc 'fix', 'Configure the Samsung Android 8 with Knox CONTAINER to disallow new admin installations.

On the MDM CONTAINER console, select the "Prevent New Admin Install" check box in the "Android Advanced Restrictions" rule.'
  impact 0.5
  ref 'DPMS Target Samsung Android OS 8 with Knox 3.x - COPE use case'
  tag check_id: 'C-80035r1_chk'
  tag severity: 'medium'
  tag gid: 'V-80363'
  tag rid: 'SV-95067r1_rule'
  tag stig_id: 'KNOX-08-014300'
  tag gtitle: 'PP-MDF-991000'
  tag fix_id: 'F-87169r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
