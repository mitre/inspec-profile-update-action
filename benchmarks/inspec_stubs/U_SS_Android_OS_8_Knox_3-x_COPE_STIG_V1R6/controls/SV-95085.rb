control 'SV-95085' do
  title 'The Samsung Android 8 with Knox CONTAINER must be configured to not allow backup of [all applications, configuration data] to remote systems: Disable Allow Google Accounts Auto Sync.'
...
SFR ID: FMT_SMF_EXT.1.1 #40"
  desc 'check', 'Review Samsung Android 8 with Knox CONTAINER configuration settings to determine if the capability to back up to a remote system has been disabled. 
...
If the MDM console "Allow Google Accounts Auto Sync" check box is selected or on the Samsung Android 8 with Knox device CONTAINER, the user can enable "Back up my data", this is a finding.
...
If the "Application disable list" configuration in the MDM console does not contain all pre-installed public cloud backup applications or if the user is able to successfully launch an application on this list, this is a finding.'
  desc 'fix', 'Configure the Samsung Android 8 with Knox CONTAINER to disable backup to remote systems (including commercial clouds).
...
2. List all pre-installed public cloud backup applications in the application disable list.'
  ...
  tag check_id: 'C-80053r1_chk'
  tag severity: 'medium'
  tag gid: 'V-80381'
  tag rid: 'SV-95085r1_rule'
  tag stig_id: 'KNOX-08-017200'
  tag gtitle: 'PP-MDF-301230'
  tag fix_id: 'F-87187r1_fix'
  tag 'documentable'
  tag cci: ['CCI-002338']
  tag nist: ['AC-20 (3)']
end
