control 'SV-94995' do
  title 'The Samsung Android 8 with Knox CONTAINER whitelist must be configured to not include applications with the following characteristics: Allows unencrypted (or encrypted but not FIPS 140-2 validated) data sharing with other mobile devices (MDs) or printers.'
...
SFR ID: FMT_SMF_EXT.1.1 #8b'
  desc 'check', 'Review Samsung Android 8 with Knox configuration settings to determine if the mobile device CONTAINER has an application disable list configured to include applications with the following characteristics:
...
If the MDM console "Application disable list" is not properly configured or on the Samsung Android 8 with Knox device CONTAINER, the user is able to launch the applications on the list, this is a finding.

Note: Refer to the Supplemental document for additional information.'
  desc 'fix', 'Configure the Samsung Android 8 with Knox CONTAINER application disable list to include applications with the following characteristics:

- allows unencrypted (or encrypted but not FIPS 140-2 validated) data sharing with other MDs or printers.

On the MDM CONTAINER console, add all applications that allow unencrypted (or encrypted but not FIPS 140-2 validated) data sharing with other MDs or printers to the "Application disable list" setting in the "Android Applications" rule.

Note: Refer to the Supplemental document for additional information.'
  impact 0.5
  ref 'DPMS Target Samsung Android OS 8 with Knox 3.x - COPE use case'
  tag check_id: 'C-79963r1_chk'
  tag severity: 'medium'
  tag gid: 'V-80291'
  tag rid: 'SV-94995r1_rule'
  tag stig_id: 'KNOX-08-002700'
  tag gtitle: 'PP-MDF-301100'
  tag fix_id: 'F-87097r1_fix'
  tag 'documentable'
