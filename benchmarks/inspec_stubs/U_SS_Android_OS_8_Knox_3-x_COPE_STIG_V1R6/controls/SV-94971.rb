control 'SV-94971' do
  title 'Samsung Android 8 with Knox must implement the management setting: CONTAINER Account whitelist.'
...
SFR ID: FMT_SMF_EXT.1.1 #47'
  desc 'check', 'Review Samsung Android 8 with Knox configuration settings to determine if the mobile device is enforcing CONTAINER Account Whitelisting. 
...
8. Verify the email account cannot be added.
...
If the MDM console "Account whitelist" is not set to contain DoD-approved email domains or on the Samsung Android 8 with Knox device, the user is able to successfully configure the email account with a domain not approved by DoD, or if the user is not able to install the DoD-approved email account, this is a finding.'
  desc 'fix', 'Configure Samsung Android 8 with Knox to enforce CONTAINER Account Whitelisting.
...
Note: Recommended to add .*@mail.mil.'
  ...
  tag check_id: 'C-79939r1_chk'
  tag severity: 'medium'
  tag gid: 'V-80267'
  tag rid: 'SV-94971r1_rule'
  tag stig_id: 'KNOX-08-000300'
  tag gtitle: 'PP-MDF-991000'
  tag fix_id: 'F-87073r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
