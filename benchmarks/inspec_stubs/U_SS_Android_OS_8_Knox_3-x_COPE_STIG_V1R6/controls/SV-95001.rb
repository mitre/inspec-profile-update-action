control 'SV-95001' do
  title 'Samsung Android 8 with Knox must be configured to: Add the MDM Client application to the CONTAINER Battery optimizations modes Whitelist.'
...
SFR ID: FMT_SMF_EXT.1.1 #47'
  desc 'check', 'Review Samsung Android 8 with Knox configuration settings to determine if the mobile device is configured to add the MDM Client application to the CONTAINER Battery optimizations modes Whitelist.
...
If the MDM CONTAINER console "Battery optimizations modes Whitelist" does not contain the MDM Client, this is a finding.'
  desc 'fix', 'Configure Samsung Android 8 with Knox to add the MDM Client application to the CONTAINER Battery optimizations modes Whitelist.

On the MDM CONTAINER console, add the MDM Client Package name to the "Battery optimizations modes Whitelist" in the "Android Applications" rule.
...
Note: Some MDM consoles may require (or take as an optional input) the MDM Client Signature.'
  ...
  tag check_id: 'C-79969r1_chk'
  tag severity: 'low'
  tag gid: 'V-80297'
  tag rid: 'SV-95001r1_rule'
  tag stig_id: 'KNOX-08-003300'
  tag gtitle: 'PP-MDF-991000'
  tag fix_id: 'F-87103r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
