control 'SV-95105' do
  title 'The Samsung Android 8 with Knox CONTAINER must implement the management setting: Install DoD root and intermediate PKI certificates on the device.'
...
SFR ID: FMT_SMF_EXT.1.1 #47'
  desc 'check', 'Review Samsung Android 8 with Knox CONTAINER configuration settings to determine if the mobile device has the DoD root and intermediate PKI certificates installed. 
...
If the MDM console "Android Certificate" does not have the DoD root and intermediate PKI certificates present or on the Samsung Android 8 with Knox device CONTAINER, "View security certificates" does not have the DoD root and intermediate PKI certificates present, this is a finding.'
  desc 'fix', 'Configure the Samsung Android 8 with Knox CONTAINER to install DoD root and intermediate certificates.

On the MDM CONTAINER console, add the PEM encoded representations of the DoD root and intermediate certificates to the certificate whitelist in the "Android Certificate" rule.

The current DoD root and intermediate PKI certificates may be obtained in self-extracting zip files at http://iase.disa.mil/pki-pke (for NIPRNet) or 
http://iase.rel.disa.smil.mil/pki-pke/function_pages/tools.html (for SIPRNet).'
  impact 0.5
  ref 'DPMS Target Samsung Android OS 8 with Knox 3.x - COPE use case'
  tag check_id: 'C-80073r1_chk'
  tag severity: 'medium'
  tag gid: 'V-80401'
  tag rid: 'SV-95105r1_rule'
  tag stig_id: 'KNOX-08-019500'
  tag gtitle: 'PP-MDF-991000'
  tag fix_id: 'F-87207r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
