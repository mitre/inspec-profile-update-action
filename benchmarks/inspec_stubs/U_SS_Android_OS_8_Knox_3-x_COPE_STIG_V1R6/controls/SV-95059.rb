control 'SV-95059' do
  title 'The Samsung Android 8 with Knox CONTAINER must be configured to disable automatic transfer of diagnostic data to an external device other than an MDM service with which the device has enrolled: Disable Google Usage and diagnostics.'
...
SFR ID: FMT_SMF_EXT.1.1#47a'
  desc 'check', 'Review Samsung Android 8 with Knox CONTAINER configuration settings to determine if the device CONTAINER disables automatic transfer of diagnostic data to an external server other than an MDM service with which the device has enrolled.
...
This validation procedure covers the third of these steps. This validation procedure is performed on the Samsung Android 8 with Knox CONTAINER only.

On the Samsung Android 8 with Knox CONTAINER, do the following:
1. Open the workspace settings.
2. Select "Google".
3. Select "Usage & diagnostics" in the overflow menu.
4. Verify the setting is off.

If the Samsung Android 8 with Knox CONTAINER "Usage & diagnostics" setting is enabled, this is a finding. 
...
Configure the mobile operating system CONTAINER to disable Google Usage & diagnostics.
1. Open the workspace settings.
2. Select "Google".
3. Select "Usage & diagnostics" in the overflow menu.
4. Uncheck the setting.'
  ...
  tag check_id: 'C-80027r1_chk'
  tag severity: 'low'
  tag gid: 'V-80355'
  tag rid: 'SV-95059r1_rule'
  tag stig_id: 'KNOX-08-013700'
  tag gtitle: 'PP-MDF-301270'
  tag fix_id: 'F-87161r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
end
