...
The PASSWORD_LIFE_TIME parameter defines the number of days a password remains valid. This can, but must not be, set to UNLIMITED. Further, the PASSWORD_GRACE_TIME parameter, if set to UNLIMITED, can nullify the PASSWORD_LIFE_TIME. PASSWORD_GRACE_TIME must be set to â€œ0â€? days (or another small integer).
...
(Note: Although the DoD requirement is for a password change every 60 days, using a value of 35 facilitates the use of PASSWORD_LIFE_TIME as a means of locking accounts inactive for 35 days. But if â€œ35â€? is not a practical or acceptable limit for password lifetime, set it to the standard DoD value of 60.)

Where a password lifetime longer than 60 is needed, document the reasons and obtain ISSO approval.'
  impact 0.5
  ref 'DPMS Target Oracle Database 12c'
  tag check_id: 'C-40954r708387_chk'
  tag severity: 'medium'
  tag gid: 'V-237735'
  tag rid: 'SV-237735r708389_rule'
  tag stig_id: 'O121-C2-015200'
  tag gtitle: 'SRG-APP-000164-DB-000401'
  tag fix_id: 'F-40917r708388_fix'
  tag 'documentable'
