...
The iprutils package provides a suite of utilities to manage and configure SCSI devices supported by the ipr SCSI storage device driver.'
  desc 'check', 'Verify the iprutils package has not been installed on the system with the following commands:
...
If the iprutils package is installed and is not documented with the Information System Security Officer (ISSO) as an operational requirement, this is a finding.'
  desc 'fix', 'Document the iprutils package with the ISSO as an operational requirement or remove it from the system with the following command:

$ sudo yum remove iprutils'
  impact 0.5
  ref 'DPMS Target Red Hat Enterprise Linux 8'
  tag check_id: 'C-33229r568426_chk'
  tag severity: 'medium'
  tag gid: 'V-230560'
