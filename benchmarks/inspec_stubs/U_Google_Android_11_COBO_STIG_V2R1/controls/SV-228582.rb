control 'SV-228582' do
  title 'Google Android 11 must be configured to disable Bluetooth or configured via User Based Enforcement (UBE) to allow Bluetooth for only Headset Profile (HSP), HandsFree Profile (HFP), and Serial Port Profile (SPP).'
...
If the AO has approved the use of Bluetooth, and Bluetooth devices using unauthorized Bluetooth profiles are listed on the device under "Connected devices", this is a finding.'
  desc 'fix', 'Configure the Google Android 11 device to disable Bluetooth or if the AO has approved the use of Bluetooth (for example, for car hands-free use), train the user to connect to only authorized Bluetooth devices using only HSP, HFP, or SPP Bluetooth capable devices (UBE).
...
The user training requirement is satisfied in requirement GOOG-11-008700.'
  ...
  tag check_id: 'C-30817r505571_chk'
  tag severity: 'low'
  tag gid: 'V-228582'
  tag rid: 'SV-228582r852656_rule'
  tag stig_id: 'GOOG-11-001400'
  tag gtitle: 'PP-MDF-301110'
  tag fix_id: 'F-30794r505572_fix'
  tag 'documentable'
