control 'SV-85653' do
  ...
  desc 'check', %q(Verify the policy value for User Configuration -> Administrative Templates -> Microsoft Excel 2016 -> Excel Options -> Security -> Trust Center -> Protected View "Set document behavior if file validation fails" is set to "Disabled".  The option 'Enabled: Open in Protected View' and Unchecked for 'Do not allow edit' is also an acceptable value.
...
If the value DisableEditFromPV is set to REG_DWORD = 1, this is not a finding.  If the value is set to REG_DWORD = 0, this is a finding.)
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Excel 2016 -> Excel Options -> Security -> Trust Center -> Protected View "Set document behavior if file validation fails" to "Disabled".'
  impact 0.5
  ref 'DPMS Target Microsoft Excel 2016'
  tag check_id: 'C-71457r5_chk'
  tag severity: 'medium'
  tag gid: 'V-71029'
  tag rid: 'SV-85653r1_rule'
  tag stig_id: 'DTOO292'
  tag gtitle: 'SRG-APP-000210'
  tag fix_id: 'F-77361r2_fix'
  tag 'documentable'
  tag cci: ['CCI-001170']
  tag nist: ['SC-18 (4)']
end
