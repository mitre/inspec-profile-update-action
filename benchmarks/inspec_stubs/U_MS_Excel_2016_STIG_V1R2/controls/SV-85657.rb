control 'SV-85657' do
  ...
  desc 'check', 'Verify the policy value for User Configuration -> Administrative Templates -> Microsoft Excel 2016 -> Application Settings -> Security ->  Trust Center -> "VBA macro Notification Settings" is set to "Enabled: Disable all with notification".  The options ‘Enabled: Disable all except digitally signed macros’ and ‘Enabled: Disable all without notification’ are more restrictive and also acceptable values. 
...
Criteria: If the value vbawarnings is REG_DWORD = 2, this is not a finding.   Values of REG_DWORD = 3 or 4 are also acceptable values.    If the registry key does not exist or the value is REG_DWORD = 1, then this is a finding.'
  desc 'fix', 'Set the policy value for User Configuration -> Administrative Templates -> Microsoft Excel 2016 -> Excel Options -> Security -> Trust Center "VBA Macro Notification Settings" to "Enabled: Disable all with notification".'
  impact 0.5
  ref 'DPMS Target Microsoft Excel 2016'
  tag check_id: 'C-71461r3_chk'
  tag severity: 'medium'
  tag gid: 'V-71033'
  tag rid: 'SV-85657r1_rule'
  tag stig_id: 'DTOO304'
  tag gtitle: 'SRG-APP-000141'
  tag fix_id: 'F-77365r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
end
