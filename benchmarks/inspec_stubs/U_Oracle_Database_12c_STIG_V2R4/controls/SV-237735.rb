...
Review DBMS settings to determine if passwords must be changed periodically. Run the following script:
    ...
    SELECT profile,
     max(decode(resource_name, 'PASSWORD_LIFE_TIME', decode(limit, 'UNLIMITED', 3000, limit)))  +
     max(decode(resource_name, 'PASSWORD_GRACE_TIME', decode(limit, 'UNLIMITED', 3000, limit))) "EFFECTIVE_TIME"
  FROM dba_profiles
...
If the EFFICTIVE_TIME is greater than 60 for any profile applied to user accounts, and the need for this has not been documented and approved by the ISSO, this is a finding.
...
Where a password lifetime longer than 60 is needed, document the reasons and obtain ISSO approval.'
  ...
  tag check_id: 'C-40954r822489_chk'
  tag severity: 'medium'
  tag gid: 'V-237735'
  tag rid: 'SV-237735r822491_rule'
