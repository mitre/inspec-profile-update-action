control 'SV-252621' do
  title 'The IBM Aspera High-Speed Transfer Endpoint must not store group content-protection secrets in plain text.'
...
Aspera recommends that you do not store content-protection secrets in aspera.conf.'
  desc 'check', 'Verify the IBM High-Speed Transfer Endpoint does not store group content-protection secrets in plain text.
...
If the "transfer_encryption_content_protection_secret" is not "AS_NULL", this is a finding.'
  desc 'fix', 'Configure the IBM High-Speed Transfer Endpoint to not store group content-protection secrets in plain text.

For each group, remove any secrets from the /opt/aspera/aspera.conf file with the following command:

$ sudo /opt/aspera/bin/asconfigurator -x "set_group_data; group_name,<groupname>; transfer_encryption_content_protection_secret,AS_NULL"'
  impact 0.5
  ref 'DPMS Target IBM Aspera Platform 4.2'
  tag check_id: 'C-56077r818031_chk'
  tag severity: 'medium'
  tag gid: 'V-252621'
  tag rid: 'SV-252621r831521_rule'
  tag stig_id: 'ASP4-TE-030190'
  tag gtitle: 'SRG-NET-000512-ALG-000062'
  tag fix_id: 'F-56027r818032_fix'
  tag 'documentable'
