control 'SV-252620' do
  title 'The IBM Aspera High-Speed Transfer Endpoint must limit the number of concurrent sessions to an organization-defined number for all accounts and/or account types.'
...
The number of incoming transfer requests to the IBM Aspera High-Speed Transfer Endpoints permitted via a POST to the REST service can be limited by the setting of "transfer_manager_max_concurrent_sessions" in The IBM Aspera.conf.'
  desc 'check', 'Verify the IBM Aspera High-Speed Transfer Endpoint limits the number of concurrent sessions to an organization-defined number for all accounts and/or account types with the following command:

$ sudo /opt/aspera/bin/asuserdata -a | grep concurrent

transfer_manager_max_concurrent_sessions: "20"

If the value returned (in this example 20 is the default) is not an organization-defined number, this is a finding.'
  desc 'fix', 'Configure the IBM Aspera High-Speed Transfer Endpoint to limit the number of concurrent sessions to an organization-defined number for all accounts and/or account types with the following command:
...
$ sudo systemctl restart asperanoded.service'
  ...
  tag check_id: 'C-56076r818028_chk'
  tag severity: 'medium'
  tag gid: 'V-252620'
  tag rid: 'SV-252620r818030_rule'
  tag stig_id: 'ASP4-TE-030180'
  tag gtitle: 'SRG-NET-000053-ALG-000001'
  tag fix_id: 'F-56026r818029_fix'
  tag 'documentable'
  tag cci: ['CCI-000054']
  tag nist: ['AC-10']
end
