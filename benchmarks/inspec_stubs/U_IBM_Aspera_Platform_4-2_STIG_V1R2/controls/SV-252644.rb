control 'SV-252644' do
  title 'The IBM Aspera High-Speed Transfer Server must restrict users read, write, and browse permissions by default.'
...
The IBM Aspera High Speed Transfer Server inherently uses file and group ownership of files and directories to support authorization for all supported operating systems. As an additional step and security best practice, ensure all transfers in or out of the authenticated connection are configured to be controlled based on privileges granted to specific users and groups within IBM Aspera configuration.'
  desc 'check', %q(Verify the IBM Aspera High-Speed Transfer Server restricts users read, write, and browse permissions by default with the following command:
...
If no results are returned or if the results produce a "true" value, this is a finding.)
  desc 'fix', 'Configure the IBM Aspera High-Speed Transfer Server to restrict users read, write, and browse permissions by default with the following commands:
...
$ sudo systemctl restart asperanoded.service'
  ...
  tag check_id: 'C-56100r818100_chk'
  tag severity: 'medium'
  tag gid: 'V-252644'
  tag rid: 'SV-252644r818102_rule'
  tag stig_id: 'ASP4-TS-020280'
  tag gtitle: 'SRG-NET-000015-ALG-000016'
  tag fix_id: 'F-56050r818101_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
