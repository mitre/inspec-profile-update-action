control 'SV-252557' do
  title 'The IBM Aspera Platform must not have unnecessary services and functions enabled.'
...
Next Generation ALGs (NGFW) and Unified Threat Management (UTM) ALGs integrate functions which have been traditionally separated. These products integrate content filtering features to provide more granular policy filtering. There may be operational drawbacks to combining these services into one device. Another issue is that NGFW and UTM products vary greatly with no current definitive industry standard.'
  desc 'check', 'Verify that only mission essential features are in use. Interview the systems administrator to determine if the following Aspera features are in use:

Aspera Shares 
Aspera Faspex 

If either Aspera Shares or Aspera Faspex are in use and are not documented with the ISSM as a mission requirement, this is a finding.'
  desc 'fix', 'Ensure all mission required features of Aspera are documented with the ISSM.'
  impact 0.5
  ref 'DPMS Target IBM Aspera Platform 4.2'
  tag check_id: 'C-56013r817839_chk'
  tag severity: 'medium'
  tag gid: 'V-252557'
  tag rid: 'SV-252557r817841_rule'
  tag stig_id: 'ASP4-00-010110'
  tag gtitle: 'SRG-NET-000131-ALG-000085'
  tag fix_id: 'F-55963r817840_fix'
  tag 'documentable'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
end
