control 'SV-252637' do
  title 'The IBM Aspera High-Speed Transfer Server must not store group content-protection secrets in plain text.'
...
Aspera recommends that you do not store content-protection secrets in aspera.conf.'
  desc 'check', 'Verify the IBM High-Speed Transfer Server does not store group content-protection secrets in plain text.
...
If the "transfer_encryption_content_protection_secret" is not "AS_NULL", this is a finding.'
  desc 'fix', 'Configure the IBM High-Speed Transfer Server to not store group content-protection secrets in plain text.

Remove any secrets from the /opt/aspera/aspera.conf file with the following command:

$ sudo /opt/aspera/bin/asconfigurator -x "set_group_data; group_name,<name>; transfer_encryption_content_protection_secret,AS_NULL"'
  impact 0.5
  ref 'DPMS Target IBM Aspera Platform 4.2'
  tag check_id: 'C-56093r818079_chk'
  tag severity: 'medium'
  tag gid: 'V-252637'
  tag rid: 'SV-252637r831529_rule'
  tag stig_id: 'ASP4-TS-020210'
  tag gtitle: 'SRG-NET-000512-ALG-000062'
  tag fix_id: 'F-56043r818080_fix'
  tag 'documentable'
