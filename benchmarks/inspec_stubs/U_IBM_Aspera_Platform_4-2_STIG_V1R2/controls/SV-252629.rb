control 'SV-252629' do
  title 'The IBM Aspera High-Speed Transfer Server must be configured to protect the authenticity of communications sessions.'
...
This requirement focuses on communications protection for the application session rather than for the network packet and establishes grounds for confidence at both ends of communications sessions in ongoing identities of other parties and in the validity of information transmitted. Depending on the required degree of confidentiality and integrity, web services/SOA will require the use of mutual authentication (two-way/bidirectional).'
  desc 'check', 'For implementations using IBM Aspera High-Speed Transfer Server, check for a <ssh_host_key_fingerprint> entry within the <server> section within The IBM Aspera High-Speed Transfer Server installation configuration file at /opt/aspera/etc/aspera.conf using the following command:
...
If the certificate is not DoD issued, this is a finding.'
  desc 'fix', %q(For implementations using the IBM Aspera High Speed Transfer Server, configure the host key fingerprint using the following procedure:
...
$ sudo systemctl restart asperanoded.service)
  ...
  tag check_id: 'C-56085r818055_chk'
  tag severity: 'medium'
  tag gid: 'V-252629'
  tag rid: 'SV-252629r818057_rule'
  tag stig_id: 'ASP4-TS-020120'
  tag gtitle: 'SRG-NET-000230-ALG-000113'
  tag fix_id: 'F-56035r818056_fix'
  tag 'documentable'
  tag cci: ['CCI-001184']
  tag nist: ['SC-23']
end
