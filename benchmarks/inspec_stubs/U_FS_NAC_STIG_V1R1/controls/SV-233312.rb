control 'SV-233312' do
  title 'If a device requesting access fails Forescout policy assessment, Forescout must communicate with other components and the switch to either terminate the session or redirect the endpoint to the remediation subnet.'
  ...
  desc 'check', 'Verify Forescout is configured to filter the policy assessment devices based on risk and are remediated accordingly based on local SSP. Verify filters for the policy assessment devices are set to take remediation actions.
...
If Forescout does not communicate with the remote access gateway to implement a policy to either terminate the session or redirect the session for endpoint remediation, this is a finding.'
  desc 'fix', 'Configure Forescout policy to take remediation actions on endpoints with risk.
...
2. Select a policy, then click Edit.
 3. Configure the Compliance Policies to include any of the following actions:
  ...
  tag check_id: 'C-36507r615850_chk'
  tag severity: 'high'
  tag gid: 'V-233312'
  tag rid: 'SV-233312r615851_rule'
  tag stig_id: 'FORE-NC-000040'
  tag gtitle: 'SRG-NET-000015-NAC-000060'
  tag fix_id: 'F-36472r605640_fix'
  tag 'documentable'
  tag cci: ['CCI-000213']
  tag nist: ['AC-3']
end
