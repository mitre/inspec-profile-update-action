control 'SV-233330' do
  title 'Forescout switch module must only allow a maximum of one registered MAC address per access port.'
...
If this is not a teleworking remote location, this exemption does not apply."
  desc 'check', 'Review the switch configuration to verify each access port is configured for a single registered MAC address.
...
If Forescout switch is not configured to permit a maximum of one registered MAC address per access port, this is a finding.'
  desc 'fix', 'Forescout has the ability to configure the amount of Maximum connected endpoints per port.
...
3. Set the Maximum connected endpoints per port to one.'
  ...
  tag check_id: 'C-36525r616541_chk'
  tag severity: 'medium'
  tag gid: 'V-233330'
  tag rid: 'SV-233330r616542_rule'
  tag stig_id: 'FORE-NC-000240'
  tag gtitle: 'SRG-NET-000343-NAC-001480'
  tag fix_id: 'F-36490r605694_fix'
  tag 'documentable'
  tag cci: ['CCI-001958']
  tag nist: ['IA-3']
end
