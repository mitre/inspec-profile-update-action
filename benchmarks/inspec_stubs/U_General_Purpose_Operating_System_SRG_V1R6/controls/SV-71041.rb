control 'SV-71041' do
  title 'The operating system must request data origin authentication verification on the name/address resolution responses the system receives from authoritative sources.'
...
This is not applicable if DNSSEC is not implemented on the local network."
  desc 'check', 'Verify the operating system requests data origin authentication verification on the name/address resolution responses the system receives from authoritative sources. If it does not, this is a finding.'
  desc 'fix', 'Configure the operating system to request data origin authentication verification on the name/address resolution responses the system receives from authoritative sources.'
  impact 0.5
  ref 'DPMS Target SRG-OS-GPOS'
  tag check_id: 'C-57351r1_chk'
  tag severity: 'medium'
  tag gid: 'V-56781'
  tag rid: 'SV-71041r1_rule'
  tag stig_id: 'SRG-OS-000399-GPOS-00178'
  tag gtitle: 'SRG-OS-000399-GPOS-00178'
  tag fix_id: 'F-61677r1_fix'
  tag 'documentable'
  tag cci: ['CCI-002465']
  tag nist: ['SC-21']
end
