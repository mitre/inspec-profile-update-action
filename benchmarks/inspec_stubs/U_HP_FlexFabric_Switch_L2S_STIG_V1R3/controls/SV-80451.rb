control 'SV-80451' do
  ...
  desc 'check', 'Review the HP FlexFabric Switch configuration to determine if services or functions not required for operation, or not related to switch functionality, are enabled.
...
Note: When Telnet server is enabled, the output for this command is telnet server enable.'
  desc 'fix', 'Remove unneeded services and functions from the HP FlexFabric Switch. Removal is recommended since the service or function may be inadvertently enabled otherwise. However, if removal is not possible, disable the service or function.
...
Note: By default, both FTP and Telnet services are disabled.'
  ...
  tag check_id: 'C-66609r1_chk'
  tag severity: 'medium'
  tag gid: 'V-65961'
  tag rid: 'SV-80451r1_rule'
  tag stig_id: 'HFFS-L2-000001'
  tag gtitle: 'SRG-NET-000131-L2S-000014'
  tag fix_id: 'F-72037r1_fix'
  tag 'documentable'
  tag cci: ['CCI-000381']
  tag nist: ['CM-7 a']
end
