...
3. Modify settings on a device'
  desc 'check', 'From the vSphere Web Client right-click the Virtual Machine and go to Edit Settings >> VM Options >> Advanced >> Configuration Parameters >> Edit Configuration. Verify the isolation.device.connectable.disable value is set to true.
...
If the virtual machine advanced setting isolation.device.connectable.disable does not exist or is not set to true, this is a finding.'
  desc 'fix', 'From the vSphere Web Client right-click the Virtual Machine and go to Edit Settings >> VM Options >> Advanced >> Configuration Parameters >> Edit Configuration. Find the isolation.device.connectable.disable value and set it to true. If the setting does not exist, add the Name and Value setting at the bottom of screen.
...
Get-VM "VM Name" | Get-AdvancedSetting -Name isolation.device.connectable.disable | Set-AdvancedSetting -Value true'
  ...
  tag check_id: 'C-40314r640120_chk'
  tag severity: 'medium'
  tag gid: 'V-237095'
  tag rid: 'SV-237095r640122_rule'
  tag stig_id: 'VMCH-65-000037'
  tag gtitle: 'SRG-OS-000480-VMM-002000'
  tag fix_id: 'F-40277r640121_fix'
  tag 'documentable'
  tag legacy: ['SV-104459', 'V-94629']
  tag cci: ['CCI-000366']
  tag nist: ['CM-6 b']
end
