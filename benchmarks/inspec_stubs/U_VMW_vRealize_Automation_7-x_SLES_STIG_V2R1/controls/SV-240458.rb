...
Requiring individuals to be authenticated with an individual authenticator prior to using a group authenticator allows for traceability of actions, as well as adding an additional level of protection of the actions that can be taken with group account knowledge.)
  desc 'check', 'Verify the SLES for vRealize prevents direct logons to the "root" account by running the following command:

# grep root /etc/shadow | cut -d "":"" -f 2

If the returned message contains any text, this is a finding.'
  desc 'fix', 'Configure the SLES for vRealize to prevent direct logons to the "root" account by performing the following operations:
...
# passwd -d root'
  ...
  tag check_id: 'C-43691r671113_chk'
  tag severity: 'high'
  tag gid: 'V-240458'
  tag rid: 'SV-240458r671115_rule'
  tag stig_id: 'VRAU-SL-000705'
  tag gtitle: 'SRG-OS-000109-GPOS-00056'
  tag fix_id: 'F-43650r671114_fix'
  tag 'documentable'
  tag legacy: ['SV-100343', 'V-89693']
  tag cci: ['CCI-000770']
  tag nist: ['IA-2 (5)']
end
