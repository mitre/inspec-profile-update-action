...
The session lock is implemented at the point where session activity can be determined and/or controlled."
  desc 'check', 'Verify the SLES for vRealize initiates a session lock after a 15-minute period of inactivity for SSH. 
...
If this is not set, this is a finding.'
  desc 'fix', "Configure the SLES for vRealize to initiate a session lock after a 15-minute period of inactivity for SSH.
...
# sed -i 's/^.*\\bClientAliveInterval\\b.*$/ClientAliveInterval 900/' /etc/ssh/sshd_config; sed -i 's/^.*\\bClientAliveCountMax\\b.*$/ClientAliveCountMax 0/' /etc/ssh/sshd_config"
  ...
  tag check_id: 'C-43584r670792_chk'
  tag severity: 'medium'
  tag gid: 'V-240351'
  tag rid: 'SV-240351r670794_rule'
  tag stig_id: 'VRAU-SL-000055'
  tag gtitle: 'SRG-OS-000029-GPOS-00010'
  tag fix_id: 'F-43543r670793_fix'
  tag 'documentable'
  tag legacy: ['SV-100129', 'V-89479']
  tag cci: ['CCI-000057']
  tag nist: ['AC-11 a']
end
